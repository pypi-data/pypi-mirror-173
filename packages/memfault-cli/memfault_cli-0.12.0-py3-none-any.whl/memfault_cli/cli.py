...
@cli.command(name="upload-debug-data-recording")
@click.option(
    "--hardware-version",
    type=click.STRING,
    required=True,
...
@click.option(
    ...
    help="Timestamp of when the debug data recording started. The timezone needs to be specified. Defaults to the current time. Example: '2022-10-14T10:33:23Z' or '2022-10-14T10:33:23+0200'.",
)
@click.option(
    "--duration-secs",
    required=True,
    type=click.INT,
    help="The duration (in seconds) of the debug data recording",
)
@click.option(
    "--mimetype",
    required=True,
...
@click.option(
    ...
    required=True,
    type=click.STRING,
    help="The reason this debug data recording was uploaded",
)
@click_argument_path
@pass_memfault_cli_context
def upload_debug_data_recording(ctx: MemfaultCliClickContext, path: str, **kwargs):
    """Upload a debug data recording (read: any file that might help you with debugging)."""
    ctx.obj.update(**kwargs)
    _do_upload_or_raise(ctx, path, DebugDataRecordingUploader)
