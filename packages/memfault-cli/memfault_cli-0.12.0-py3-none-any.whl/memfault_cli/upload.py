...
class DebugDataRecordingUploader(Uploader):
    authenticator_types = [
        ProjectKeyAuthenticator,
        OrgTokenAuthenticator,
        BasicAuthenticator,
    ...
    def entity_url(self) -> str:
        if self.authenticator.project_key_auth():
            return f"{self._api_base_url()}/upload/debug-data-recording"
        else:
            return f"{self._projects_base_url()}/debug-data-recording"

    def post_token_extra_request_data(self) -> Optional[Dict]:
        device_traits = self.ctx.device_traits_info
        info = self.ctx.debug_data_recording_info
        return {
