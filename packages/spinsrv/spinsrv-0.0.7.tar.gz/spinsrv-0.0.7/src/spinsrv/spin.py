...
class Key:
    ...
    def unmarshal_json(self, j: dict):
        ...
        if "Meta" in j:
            self.meta = j["Meta"]
        if "CreatedAt" in j:
            self.created_at = dateutil.parser.isoparse(j["CreatedAt"])
        if "ExpiresAt" in j:
            self.expires_at = dateutil.parser.isoparse(j["ExpiresAt"])
...
class DirEntry:
    ...
    def unmarshal_json(self, j: dict):
        ...
        if "Path" in j:
            self.path = j["Path"]
        if "Time" in j:
            self.time = dateutil.parser.isoparse(j["Time"])
        if "Sequence" in j:
...
@dataclasses.dataclass
class DirOp:
    operation: DirOperation = dataclasses.field(default_factory=DirOperation)
...
@dataclasses.dataclass
class BitOp:
    operation: BitOperation = dataclasses.field(default_factory=BitOperation)
    ref: Ref = ""
    bytes: bytes = b""
