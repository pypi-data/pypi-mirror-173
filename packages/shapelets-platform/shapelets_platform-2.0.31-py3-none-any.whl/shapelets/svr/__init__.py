...
def setup_http_server(cfg: Settings, blocking: bool, pid_file: Optional[str] = None) -> Optional[InProcServer]:
    """
...
class SharedState:
    __slots__ = ('__services', '__local_server')
...
def _run_out_of_process():
    ...
    """
    settings = Settings()
    setup_http_server(settings, True)
...
def initialize_svr(server_mode: Optional[ServerModeType] = None, **kwargs):
    ...
    if server_mode == 'out-of-process':
        ...
        this._state.services = setup_remote_client(settings)
        ...
        setup_http_server(settings, True, kwargs.get('pid_file', None))
        ...
        this._state.local_server = setup_http_server(settings, False)
        # build the proxies
        this._state.services = setup_remote_client(settings)
    else:
        # run in headless mode
        this._state.services = setup_headless(settings)
        ...
        if not server_up and server_mode == 'out-of-process':
            ...
            with open(log_file, "wb") as outfile:
                ...
                if os.name == 'nt':
                    ...
                    params['creationflags'] = flags
                else:
                    params['start_new_session'] = True

                process = subprocess.Popen(args, **params)
                warnings.warn(f'Server process launched with pid {process.pid}.  Log file {log_file}', UserWarning)
