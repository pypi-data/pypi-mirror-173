...
def deploy_xe_infra(device_list: DeviceDict, usecase: Automation) -> None:
    ...
    try:
        ...
        w2 = path.join(basedir_infr_files, "ddr_python-1.0.8-py3-none-any.whl")
        ...
        for device in flow.active_devices.values():
            ...
            pip_file = device.scp_upload_from_file(
                pipfiles, "bootflash:/guest-share/ddr/pipfiles.sh"
            ).wait()
            debug("pipfiles.sh done")

            xe_commands = device.scp_upload_from_file(
                xe_commands, "bootflash:/guest-share/ddr/xe_commands.py"
            ).wait()
            debug("xe_commands.py done")

# requires pip install after updating pip in section below using shell script
            ...
            w2_file = device.scp_upload_from_file(
                w2, "bootflash:/guest-share/ddr/ddr_python-1.0.8-py3-none-any.whl"
            ).wait()
    ...
    try:
        ...
        export_ins1 = flow.exec_wait(
                [
                    "guestshell",
                    "python3 -m pip install /bootflash/guest-share/ddr/ddr_python-1.0.8-py3-none-any.whl --user",
    ...
    try:
        delete_files = flow.exec_wait(
            [
                ...
                "delete /force flash:guest-share/ddr/ddr_python-1.0.8-py3-none-any.whl",
