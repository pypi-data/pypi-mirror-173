...
class TableFrame(object):
  ...
  def _load(self, paths, header_mode = HeaderMode.NONE, headerMode = None,
            ...
            suppress_errors = False, on_duplicate_keys = 'error', **kwargs):
  ...
  def _build_arrow_table(self, data, row_labels,
    ...
    def is_numpy_float_list(x):
      for y in x:
        if isinstance(y, numpy.ndarray):
          if y.dtype == 'float64' or y.dtype == 'float32':
             return True;
          elif y.dtype == 'object':
            res = is_numpy_float_list(y)
            if res is True or res is False:
              return res
          else:
             return False
        # Zero length array, so type is unknown.
        return 0

    def is_float_list(column):
      if column.dtype == 'object' and len(column) > 0:
        for i in range(len(column)):
          ...
          if not isinstance(val, numpy.ndarray):
            break
          res = is_numpy_float_list(val)
          if res != 0:
            return res
      return False

    def is_pandas_mixed(column):
      infer_type = pandas.api.types.infer_dtype(column)
      if infer_type == 'mixed':
        # Check first value is a list.
    ...
    if is_pandas:
      ...
      for i, col in enumerate(table):
        # Pyarrow turns pandas nans into none when converting.
        # So we unpack it and convert into arrow array later.
        # To preserve this nan type.
        if table[col].dtype == 'float64' or table[col].dtype == 'float32' or is_float_list(table[col]):
          out_dict[i] = table[col].to_list()
        # If the column is detected to be mixed, just convert the column to strings.
        elif is_pandas_mixed(table[col]):
          out_dict[i] = pyarrow.Array.from_pandas(table[col].astype('string'))
        else:
          out_dict[i] = pyarrow.Array.from_pandas(table[col])
    else:
      out_dict = [[None for i in range(len(table))] for j in range(len(final_schema))]
      strings_detected = [False for j in range(len(final_schema))]
      numerical_detected = [False for j in range(len(final_schema))]
      other_detected = [False for j in range(len(final_schema))]
      date_detected = [False for j in range(len(final_schema))]
      time_detected = [False for j in range(len(final_schema))]
      datetime_detected = [False for j in range(len(final_schema))]
      correction = 0
      for i, row in enumerate(table):
        ...
        for j, val in enumerate(row):
          # Type detection for dirty data:
          if isinstance(val, str):
            strings_detected[j] = True
          elif isinstance(val, (int, float, complex)):
            numerical_detected[j] = True
          elif isinstance(val, date):
            date_detected[j] = True
          elif isinstance(val, time):
            time_detected[j] = True
          elif isinstance(val, datetime):
            datetime_detected[j] = True
          elif val != None:
            other_detected[j] = True
      ...
      for i, col in enumerate(out_dict):
        type_count = other_detected[i] + strings_detected[i] + numerical_detected[i] + \
                     date_detected[i] + time_detected[i] + datetime_detected[i]
        if type_count > 1:
          for j, val in enumerate(col):
            # For nan it doesn't equal itself.
            # We convert that to none for mixed types.
    ...
    return (pyarrow.Table.from_pydict(arrow_dict), invalid_rows)

  def _build_flight_path(self, row_labels, row_label_columns, suppress_errors,
                         source_vertex_row_labels = None, target_vertex_row_labels = None,
                         frame_to_file_column_mapping = None, on_duplicate_keys = 'skip'):
...
class VertexFrame(TableFrame):
  ...
  def load(self, paths,
    ...
    return self._load(paths, header_mode, headerMode, record_history, row_labels,
                      row_label_columns, delimiter = delimiter,
                      frame_to_file_column_mapping = frame_to_file_column_mapping,
                      suppress_errors = suppress_errors, on_duplicate_keys = on_duplicate_keys)

  def insert(self, data, row_labels = None,
             row_label_columns = None, suppress_errors = False,
             on_duplicate_keys = 'error'):
    """
    Inserts data rows. The properties of the new data must match the schema
    in both order and type.
