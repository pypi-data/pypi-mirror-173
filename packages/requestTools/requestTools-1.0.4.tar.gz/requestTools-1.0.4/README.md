...
对于常见的http request请求进行了封装，通过实现NormalHttpRequestProcessor、XXXNormalHttpRequestProcessor基类，即可轻松完成http请求，并且能获取到http请求后的页面结果数据，目前只支撑json的返回结果数据解析。
...
class XXXPageDataHttpRequestProcessor(PageDataHttpRequestProcessor):
    """一个基本的分页爬取所有页面数据的HTTP请求处理实现类
       该类重新实现了setCookieFileName,setUrl,setMethod,setFormValues,getTotal,getCurrentPageDatas,parseRowValues 8个抽象方法
    """
    ...
    def setFormValues(self, dynamicParams=None):
        ...
        #设置当前页码
        formValues["currentPage"] = dynamicParams[self.CURRENT_PAGE_NUMBER]
