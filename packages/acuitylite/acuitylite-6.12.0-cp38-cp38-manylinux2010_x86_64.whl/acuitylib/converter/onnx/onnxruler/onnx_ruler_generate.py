...
"param_map": {
    "pooling":{
        ...
        "pad": ["INTS", "CODE", "[str(p) for p in self.array_layout(self.attr_pick(node['QLinearAveragePool'],"
                                "'pads', [ 0, 0]), [0, 1])]"],
        "global_pooling": ["BOOL", "VALUE", False],
        "ksize": ["INT", "CODE", "self.attr_pick(node['QLinearAveragePool'], 'kernel_shape', [1])[0]"],
