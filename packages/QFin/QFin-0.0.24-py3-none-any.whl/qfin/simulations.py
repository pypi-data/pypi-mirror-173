...
class GeometricBrownianMotion:
    ...
    def vectorized_paths(self, S0, mu, sigma, steps, T, paths):
        # Discretized change in time
        dt = T/steps
        # Matrix of Brownian motions, and ABM/GBM price processes
        ...
        for i in range(0, steps):
            # Standardize cross-sectionally in Z to aid in theoretical convergence
            if paths > 1:
                Z[:, i] = ((Z[:, i] - np.mean(Z[:, i]))/np.std(Z[:, i]))
            X[:, i+1] = X[:, i] + (mu - .5*sigma*sigma)*dt + sigma*Z[:, i]*np.power(dt, .5)
        S = np.exp(X)  # exp of ABM
        return S

    def __init__(self, S, mu, sigma, steps, T, paths=1):
        self.simulated_paths =  self.vectorized_paths(S, mu, sigma, steps, T, paths)
        self.simulated_path = self.simulated_paths[-1]
...
class StochasticVarianceModel:
    ...
    def __init__(self, S, mu, r, div, alpha, beta, rho, vol_var, inst_var, dt, T):
        self.simulated_paths = self.simulate_path(S, mu, r, div, alpha, beta, rho, vol_var, inst_var, dt, T)
