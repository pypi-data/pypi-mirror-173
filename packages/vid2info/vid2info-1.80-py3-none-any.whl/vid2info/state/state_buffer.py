    ...
    def get_elements_time_per_state(self, class_to_state_dict: dict = {})-> dict[int, dict[str, float]]:
        ...
        for track_id, element in self.elements_history.items():
            ...
            if len(history) == 0:
                ...
                assert element[OUT_CLASS_NAME] == initial_class, f"The element has not been seen changing in the " \
                                                                    "scene, but its class has changed from " \
                                                                    f"{initial_class} to {element[OUT_CLASS_NAME]}."
                ...
                assert history[-1][NEW_STATE] == lapse_state, "The last state of the element is not the " \
                                                                "final class."
