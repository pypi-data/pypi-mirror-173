...
class DashboardSession(object):
    ...
    def process_projects(self, project_list, reprocess=False):
        """Calls :meth:`process_project` for each project in a list

        Args:
            project_list (list, str, or Project): List of project names of :class:`~ccrenew.dashboard.project.Project` objects to process.
    ...
    def process_all_projects(self):
        """Processes all projects in the :attr:`~project_list` for the :class:`~DashboardSession` instance
        """
    ...
    def pickle_project(self, project_name, year=None):
        """Saves the project to a serialized pickle in the :attr:`~ccrenew.dashboard.project.Project.project_directory`
        for the :class:`~ccrenew.dashboard.project.Project`.

        Args:
            project_name (str, Project, or list): The name of the project to pickle.
            year (int, optional): Year to save the pickle. Defaults to None.

        Raises:
            TypeError: if the wrong type is supplied to :attr:`~ccrenew.dashboard.project.Project.project_directory`.
        """
        if isinstance(project_name, list):
    ...
    def __update_project_filepaths(self, project):
        ...
        Args:
            project (Project): A :class:`~ccrenew.dashboard.project.Project`

        Returns:
            Project: a `~ccrenew.dashboard.project.Project` with updated
                filepaths
        """
    ...
    def get_project(self, project_name, get_neighbors=True):
        ...
        Returns:
            Project: the `Project` for the given the project name. If the project does not exist in the session when the call is made, the project will be initialized & returned.
        """
    ...
    def draw_plots(self, project_name, plot_order=None, *args, **kwargs):
        ...
        Args:
            project_name (str, Project, or list): A project or list of projects
            ...
            plot_order (list or str, optional): A list of plots to draw. This can
                be any number of plots from one to all plots. Defaults to all plots in the below order.
