"""
Magic methods allow custom classes to be compatible with various functions defined in `mlm.math`, analogous to how implementing `__hash__` allows objects to be used with `hash()`.
The magic methods are grouped into purely declarative classes (interfaces) by what functionality they provide.

* `Shaped` objects have a `mlm.math.Shape`.
...
class Shaped(metaclass=_ShapedType):
    ...
    In either case, the returned shape must be an instance of `mlm.math.Shape`.

    To check whether an object is `Shaped`, use `isinstance(obj, Shaped)`.

    **Usage in `mlm.math`:**

    The functions `mlm.math.shape` as well as dimension filters, such as `mlm.math.spatial` or `mlm.math.non_batch` can be called on all shaped objects.
...
class Sliceable(metaclass=_SliceableType):
    ...
    **Usage in `mlm.math`:**
    ...
    In addition to slicing, sliceable objects can be unstacked along one or multiple dimensions using `mlm.math.unstack`.
...
class Shapable(metaclass=_ShapableType):
    ...
    * `mlm.math.concat`
    * `mlm.math.expand`
    * `mlm.math.rename_dims`
    * `mlm.math.pack_dims`
    * `mlm.math.unpack_dim`
    * `mlm.math.flatten`

    Additionally, the `mlm.math.BoundDim` syntax for dimension renaming and retyping is enabled, e.g. `obj.dim.as_channel('vector')`.
    """
...
class PhiTreeNode(metaclass=_PhiTreeNodeType):
    ...
    `mlm.math.Tensor` instances as well as PyTree nodes (`tuple`, `list`, `dict` with `str` keys) are Φ-tree nodes.
    ...
    **Usage in `mlm.math`:**
    ...
    Disassembly and assembly of Φ-tree nodes uses `mlm.math.copy_with` which will call `__with_attrs__` if implemented.
    """
...
class BoundDim:
    ...
    ```python
    from mlm.math.magic import BoundDim
