...
class Tensor:
    ...
    * Tensors can have non-uniform shapes, meaning that the size of dimensions can vary along other dimensions.
    ...
    To construct a Tensor, use `mlm.math.tensor()`, `mlm.math.wrap()` or one of the basic tensor creation functions,
    ...
    def __unstack__(self, dims: Tuple[str, ...]) -> Tuple['Tensor', ...]:  # from mlm.math.magic.Sliceable
        if len(dims) == 1:
...
class CollapsedTensor(Tensor):  # package-private
    ...
    def _expand(self):
        self._cache()
        # from mlm.math import all_available
...
def tensor(data: Tensor or Shape or tuple or list or numbers.Number,
    ...
    * Jax: [`jax.numpy.array`](https://jax.readthedocs.io/en/latest/_autosummary/jax.numpy.array.html)
    ...
    See Also:
        `mlm.math.wrap()` which uses `convert=False`, `layout()`.
...
def wrap(data: Tensor or Shape or tuple or list or numbers.Number,
         *shape: Shape) -> Tensor:
    """ Short for `mlm.math.tensor()` with `convert=False`. """
...
def compatible_tensor(data, compat_shape: Shape = None, compat_natives=(), convert=False):
    if isinstance(data, Tensor):
        ...
        if compat_shape.channel_rank > 1 and len(shape) == 1 and 'vector' in compat_shape.channel:
            ...
            warnings.warn(f"Combining a mlm.math.Tensor with a {data_type} of same shape is not invariant under shape permutations. Please convert the {data_type} to a mlm.math.Tensor first. Shapes: {shape} and {compat_shape}", SyntaxWarning, stacklevel=5)
...
def disassemble_tensors(obj: Tensor or Tuple[Tensor, ...] or List[Tensor], expand: bool) -> tuple:
    ...
    """
    assert isinstance(obj, (Tensor, tuple, list)), f"jit-compiled function returned {type(obj)} but must return either a 'mlm.math.Tensor' or tuple/list of tensors."
...
def disassemble_tree(obj: PhiTreeNodeType) -> Tuple[PhiTreeNodeType, List[Tensor]]:
    ...
    Native tensors will be wrapped in mlm.math.Tensors with default dimension names and dimension types `None`.
