...
def unstack(value, dim: DimFilter):
    ...
    Args:
        value: `mlm.math.magic.Shapable`, such as `mlm.math.Tensor`
...
def stack(values: tuple or list or dict, dim: Shape, **kwargs):
    ...
    Args:
        values: Collection of `mlm.math.magic.Shapable`, such as `mlm.math.Tensor`
...
def concat(values: tuple or list, dim: str or Shape, **kwargs):
    """
    Concatenates a sequence of `mlm.math.magic.Shapable` objects, e.g. `Tensor`, along one dimension.
    The shapes of all values must be equal, except for the size of the concat dimension.

    Args:
        values: Tuple or list of `mlm.math.magic.Shapable`, such as `mlm.math.Tensor`
...
def unpack_dim(value, dim: str or Shape, unpacked_dims: Shape, **kwargs):
    ...
    Args:
        value: `mlm.math.magic.Shapable`, such as `Tensor`, for which one dimension should be split.
...
def flatten(value, flat_dim: Shape = instance('flat'), **kwargs):
    ...
    Args:
        value: `mlm.math.magic.Shapable`, such as `Tensor`.
...
PhiTreeNodeType = TypeVar('PhiTreeNodeType')  # Defined in mlm.math.magic: tuple, list, dict, custom
...
def cast(x: OtherMagicType, dtype: DType or type) -> OtherMagicType:
    ...
    * Jax: [`jax.numpy.array`](https://jax.readthedocs.io/en/latest/_autosummary/jax.numpy.array.html)
    ...
    Args:
        x: `Tensor`
        dtype: New data type as `mlm.math.DType`, e.g. `DType(int, 16)`.
