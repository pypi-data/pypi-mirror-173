...
class Shape:
    ...
    def _expand(self, dim: 'Shape', pos=None) -> 'Shape':
        """**Deprecated.** Use `mlm.math.merge_shapes()` or `mlm.math.concat_shapes()` instead. """
    ...
    def well_defined(self):
        ...
        Shapes with undefined sizes may be used in `mlm.math.tensor()`, `mlm.math.wrap()`, `mlm.math.stack()` or `mlm.math.concat()`.
    ...
    def shape(self) -> 'Shape':
        ...
        for size in self.sizes:
            if isinstance(size, math.Tensor):
    ...
    def is_non_uniform(self) -> bool:
        ...
        """
        for size in self.sizes:
            if isinstance(size, math.Tensor) and size.rank > 0:
                return True
        return False
        ...
        for name, selection in selection.items():
            ...
            if isinstance(selection, int):
                if result.is_uniform:
                    ...
                    gathered_sizes = [(s[{name: selection}] if isinstance(s, math.Tensor) else s) for s in result.sizes]
...
def shape(obj) -> Shape:
    """
    If `obj` is a `Tensor` or `mlm.math.magic.Shaped`, returns its shape.
