...
* `from mlm.flow import *`  for NumPy mode
* `from mlm.tf.flow import *`  for TensorFlow mode
* `from mlm.torch.flow import *` for PyTorch mode
* `from mlm.jax.flow import *` for Jax mode

Project homepage: https://github.com/tum-pbs/PhiFlow

Documentation overview: https://tum-pbs.github.io/PhiFlow

PyPI: https://pypi.org/project/mlmflow/
"""
...
def verify():
    ...
    To run verify without importing `mlm`, run the script `tests/verify.py` included in the source distribution.
    """
    import sys
...
def detect_backends() -> tuple:
    ...
    Returns:
        `tuple` of `mlm.math.backend.Backend`
    """
    try:
...
def set_logging_level(level='debug'):
    ...
    """
    from mlm.math.backend import PHI_LOGGER
