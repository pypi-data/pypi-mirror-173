...
Importing this module registers the TensorFlow backend with `mlm.math`.
Without this, TensorFlow tensors cannot be handled by `mlm.math` functions.

To make TensorFlow the default backend, import `mlm.tf.flow`.
"""
...
if _platform.system().lower() == 'windows':  # prevent Blas GEMM launch failed on Windows
    for i, device in enumerate(_tf.config.list_physical_devices('GPU')):
        ...
        _LOGGER.info(f"mlm.tf: Setting memory_growth on GPU {i} to True to prevent Blas errors")
