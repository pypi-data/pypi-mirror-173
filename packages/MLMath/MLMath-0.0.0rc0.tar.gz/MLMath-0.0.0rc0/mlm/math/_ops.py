...
def choose_backend_t(*values, prefer_default=False) -> Backend:
    ...
    Backends need to be registered to be available, e.g. via the global import `mlm.<backend>` or `mlm.detect_backends()`.
    ...
    Returns:
        The selected `mlm.math.backend.Backend`
    """
...
def convert(x, backend: Backend = None, use_dlpack=True):
    ...
    *Warning*: This operation breaks the automatic differentiation chain.
    ...
    See Also:
        `mlm.math.backend.convert()`.

    Args:
        x: `Tensor` to convert. If `x` is a `PhiTreeNode`, its variable attributes are converted.
        backend: Target backend. If `None`, uses the current default backend, see `mlm.math.backend.default_backend()`.
...
def all_available(*values: Tensor) -> bool:
    ...
    * Jax: `isinstance(x, jax.core.Tracer)`
    ...
    """
    from mlm.math._functional import ShiftLinTracer
    for value in values:
...
def native(value: Tensor or Number or tuple or list or Any):
    ...
    If `value` is a `mlm.math.Tensor`, this is equal to calling `mlm.math.Tensor.native()`.
...
def numpy(value: Tensor or Number or tuple or list or Any):
    ...
    If `value` is a `mlm.math.Tensor`, this is equal to calling `mlm.math.Tensor.numpy()`.
...
def reshaped_tensor(value: Any,
    ...
    See Also:
        `mlm.math.tensor()`, `reshaped_native()`, `unpack_dim()`.
    ...
    Args:
        ...
        *inputs: Uniform `Tensor` arguments
        channels_last: (Optional) Whether to put channels as the last dimension of the native representation.
            ...
            see `mlm.math.backend.Backend.prefers_channels_last()`.
...
def pad(value: Tensor, widths: dict, mode: 'e_.Extrapolation' or Tensor or Number, **kwargs) -> Tensor:
    ...
    Args:
        ...
        kwargs: Additional padding arguments.
            These are ignored by the standard extrapolations defined in `mlm.math.extrapolation` but can be used to pass additional contextual information to custom extrapolations.
            Grid classes from `mlm.field` will pass the argument `bounds: Box`.
...
def to_float(x) -> Tensor or PhiTreeNode:
    ...
    See the `mlm.math` module documentation at https://tum-pbs.github.io/PhiFlow/Math.html
...
def scatter(base_grid: Tensor or Shape,
    ...
    * Jax: [`jax.lax.scatter_add`](https://jax.readthedocs.io/en/latest/_autosummary/jax.lax.scatter_add.html), [`jax.lax.scatter`](https://jax.readthedocs.io/en/latest/_autosummary/jax.lax.scatter.html)
    ...
    if indices_gradient:
        from mlm.math import custom_gradient
...
def assert_close(*values,
    ...
    if not values:
        return
    mlm_tensors = [t for t in values if isinstance(t, Tensor)]
    if mlm_tensors:
        values = [compatible_tensor(t, mlm_tensors[0].shape)._simplify() for t in values]  # use Tensor to infer dimensions
