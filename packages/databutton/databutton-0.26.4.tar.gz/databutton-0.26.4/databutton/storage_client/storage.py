...
class RawStorage:
    ...
    def _init(self):
        ...
        if self._project_id is None:
            ...
            # TODO: How do we distinguish between user_id being a user or a system token?
            token_claims = jwt.decode(
                get_auth_token(), options={"verify_signature": False}
            )
            self._user_id: Optional[str] = token_claims.get("id")
            self._user_name: Optional[str] = token_claims.get("name")
