...
class LoadWavesurfer:
    ...
    def __init__(self, inputFile, filter=False, downsample=False, downsampling_factor=4):
        ...
        downsample : bool, optional
            Choose whether the signal should be downsampled, by default False
        downsampling_factor : int, optional
            Downsampling factor to use with `signal.decimate()`, by default 4
        """
    ...
    def stimParams(self):
        ...
        if self.inputFile.parent.stem == "20220602":
            # on 20220602 I stupidly altered the stimulation library mid-experiment, fixing a mistake
        ...
        if self.isStimEnabled():
            ...
            if selected_stim == "ws.StimulusMap":
                ...
                if length > map_metadata["Duration"]:
                    # sometimes, a dumb user like myself may incorrectly input a total map duration
                    ...
                    # stimulation) and not the actual programmed length (40s)
