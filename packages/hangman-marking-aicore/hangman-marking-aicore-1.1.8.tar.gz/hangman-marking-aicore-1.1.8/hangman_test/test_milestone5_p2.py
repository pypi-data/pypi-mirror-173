        ...
        try:
            ...
            with redirect_stdout(f):
                with patch('builtins.input', side_effect=['b', 'a', 'n']):
                    ...
                    message = f.getvalue()
            self.assertIn('You won!', message, 'The play_game function should print "You won!" when the user wins the game. If you are not sure what is wrong, try to run the play_game function with the word "banana" and the following inputs: "b", "a", "n"')
            f = io.StringIO()
            with redirect_stdout(f):
