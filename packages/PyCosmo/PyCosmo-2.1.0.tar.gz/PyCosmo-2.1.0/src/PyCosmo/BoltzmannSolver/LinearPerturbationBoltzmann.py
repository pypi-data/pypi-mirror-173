        ...
        if self._cosmo._pool is None:
            if diag_only:
                for ai, ki in zip(a, kk):
                    src_m.append(src_m_column_j((self, ai, ki)))
            else:
                for ki in kk:
                    src_m.append(src_m_column_j((self, a, ki)))
        else:
            kk = np.array(kk)
            perm = np.argsort(kk)
            kk = kk[perm]
            src_m = [None] * len(kk)
            if diag_only:
                args = [(self, ai, ki) for ai, ki in zip(a, kk)]
            else:
                args = [(self, a, ki) for ki in kk]
