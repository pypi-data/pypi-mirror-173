...
def get_instance_role(
    ...
    log_exception_method: Optional[Callable[[str], None]],
) -> str:
    """ Helper method to get instance role from cloud_resource_record. """
    instance_role = "ray-autoscaler-v1"
    if cloud_resource_record and cloud_resource_record.aws_iam_role_arns:
        for arn in cloud_resource_record.aws_iam_role_arns:
            if "cluster_node_role" in arn:
                try:
                    return arn.split("/")[1]
                except Exception:
                    if log_exception_method:
                        log_exception_method(
                            f"Error parsing aws_iam_role_arns for cloud {cloud_id}."
                        )
