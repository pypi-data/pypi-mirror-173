...
class Aruba505Driver(NetworkDriver):
    ...
    def switch_to_config_mode(self):
        self._send_command("config terminal")
        time.sleep(8)
        ...
        if config:
            ...
            if self.config_commands:
                ...
                if _running_config:
                    ...
                    print(f"The archive is:\n {self.archive}\n\n")

                    # enter config mode
                    #self.switch_to_config_mode()
                    self.device.send_command(f"config terminal", read_timeout=90)
                    print(f"Inside the config mode..........")

                    for cmd in self.config_commands:
                        self.device.send_command(cmd, read_timeout=90)
                        print(f"Sending: {cmd}\n")

                    # return to global config mode
                    #self.switch_to_safe_mode()
                    self.device.send_command(f"exit")
                    print(f"Exiting the config mode ...... hahaha")
        if filename:
