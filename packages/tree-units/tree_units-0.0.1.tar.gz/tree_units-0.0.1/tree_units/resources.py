...
class Node(BaseModel):
    ...
    def section_must_be_uniform(cls, v):
        v = str(v)
        if SECTION.search(v):
    ...
    def caption_must_be_uniform(cls, v):
        return str(v)

    @validator("content", pre=True)
    def content_must_be_stripped(cls, v):
        return v.strip()


class EventStatute(BaseModel):
    """The statute + unit item affecting a Codification unit."""

    locator: str = generic_item
    caption: str = generic_caption
    content: str = generic_content
    statute: str = Field(
        ...,
        title="Text that is parseable, see statute-utils",
        description="e.g. 'Republic Act No. 386'",
        min_length=4,
    )
    variant: int = generic_variant
...
class StatuteAffector(EventStatute):
    action: StatuteAffectorAction = Field(StatuteAffectorAction.Originated)
...
class CitationAffector(EventCitation):
    ...
    action: CitationAffectorAction = Field(CitationAffectorAction.Interpreted)
