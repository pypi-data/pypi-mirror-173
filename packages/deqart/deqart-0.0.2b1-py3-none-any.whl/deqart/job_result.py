...
class JobResult:
    ...
    def __str__(self):
        return f"Job ID: {self.job_id}, run status: {self.run_status}, queue time (ms): {self.queue_time_ms}, run time (ms): {self.run_time_ms}, worker runtime (ms): {self.worker_runtime_ms}, num qubits: {self.num_qubits}, status: {self.status}"
