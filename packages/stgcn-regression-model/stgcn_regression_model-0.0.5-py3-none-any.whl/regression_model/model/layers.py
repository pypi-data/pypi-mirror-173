...
class CausalConv1d(nn.Conv1d):
    ...
    def forward(self, input):
        ...
        if self.__padding != 0:
            return result[: , : , : -self.__padding] # Chomp1d
...
class CausalConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, enable_padding=False, dilation=1, groups=1, bias=True):
        ...
        dilation = nn.modules.utils._pair(dilation) # conventional convolutional layer is 1-dilated layer
        if enable_padding == True:
    ...
    def forward(self, input):
        if self.__padding != 0:
            input = F.pad(input, (self.left_padding[1], 0, self.left_padding[0], 0)) # (left_pad, right_pad, top_pad, bottom_pad)
...
class TemporalConvLayer(nn.Module):
    ...
    def __init__(self, Kt, c_in, c_out, n_vertex, act_func):
        ...
        self.Kt = Kt
        self.c_in = c_in # Channel input
        self.c_out = c_out # Channel output
