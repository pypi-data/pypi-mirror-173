...
class Client:
    ...
    def register_artifact(self, url, kind, scenario, name, description=""):
    ...
    def start_deployment(self, template):
    ...
    def deployment(self, deployment):
        ...
        return self.sess.get(
            urljoin(self.url, f"/v2/lm/deployments/{deployment}")
        ).json()
    ...
    def deployment_logs(self, deployment):
        ...
        if isinstance(deployment, dict):
            deployment = deployment["id"]
        logs = self.sess.get(
            urljoin(self.url, f"/v2/lm/deployments/{deployment}/logs")
        ).json()
        return "".join([x["msg"] for x in logs["data"]["result"]])
