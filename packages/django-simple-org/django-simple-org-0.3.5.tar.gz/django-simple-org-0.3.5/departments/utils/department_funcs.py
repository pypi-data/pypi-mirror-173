...
def update_department(pk: int, data: dict):
    ...
    try:
        ...
        # 将当前部门下的子级部门的path修改, 即:部门迁移,其下子级部门跟随一起迁移;
        if son_departments:
            path.append(instance.id)
            son_path = path
            Department.objects.filter(id__in=son_departments).update(path=son_path)
...
def delete_department(organization_id: int, pk: int):
    ...
    departments = list(Department.objects.filter(**dic).values_list("id", flat=True))  # 查询当前部门下,是否有子部门
    departments.append(instance.id)  # 插入要删除的部门id,
...
def mv_show_order(organization_id: int, data: dict):
    ...
    # departments列表值已经根据 show_order 进行过排序, 然后将 show_order 的值进行互换
