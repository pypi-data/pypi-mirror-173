...
class MicroserviceTestCase(HEAAioHTTPTestCase, ABC):
    ...
    def __init__(self,
                 coll: str,
                 desktop_objects: Mapping[str, list[DesktopObjectDict]] | Mapping[CollectionKey, list[DesktopObjectDict]],
        ...
        self.__desktop_objects = convert_to_collection_keys(desktop_objects)
        if content is not None:
            validate_collection_keys(content)
        self._content = convert_to_collection_keys(content) if content else None
    ...
    def run(self, result=None):
        ...
        with self._caplog.at_level(logging.DEBUG):
            with app_context(db_manager_cls=self.__db_manager_cls,
                             ...
                             other_docker_images=list(self.__other_docker_images) if self.__other_docker_images is not None else None,
...
def get_test_case_cls_default(coll: str,
                              ...
                              content: Mapping[str, dict[str, bytes]] | Mapping[CollectionKey, dict[str, bytes]] | None = None,
                              content_type: Optional[str] = None,
                              put_content_status: Optional[int] = None,
                              include_root=False,
                              href: Optional[str | URL] = None,
