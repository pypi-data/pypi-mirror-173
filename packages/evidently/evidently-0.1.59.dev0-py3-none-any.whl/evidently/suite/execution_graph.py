...
class SimpleExecutionGraph(ExecutionGraph):
    ...
    def get_metric_execution_iterator(self) -> List[Tuple[Metric, Metric]]:
        aggregated: Dict[Type[Metric], List[Metric]] = functools.reduce(
            _aggregate_metrics, self.metrics, {}
        )
        metric_to_calculations = {}
        for metric_type, metrics in aggregated.items():
            metrics_by_parameters: Dict[tuple, List[Metric]] = functools.reduce(
                _aggregate_by_parameters, metrics, {}
            )
            calculations = []
            for params, params_metrics in metrics_by_parameters.items():
                logging.debug(
                    f"{metric_type.__name__} with params ({params}): {len(params_metrics)} combined"
                )
