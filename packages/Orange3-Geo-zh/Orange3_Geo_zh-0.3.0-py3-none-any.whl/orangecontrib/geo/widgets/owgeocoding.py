...
class OWGeocoding(widget.OWWidget):
    name = '地理编码(Geocoding)'
    description = '将区域名称编码为地理坐标，或将经纬度反向地理编码为区域。'
    ...
    class Outputs:
        coded_data = Output("编码的数据(Coded Data)", Table, default=True, replaces=['Coded Data'])

    settings_version = 2
    settingsHandler = settings.DomainContextHandler()

    ID_TYPE = OrderedDict((
        ('国名', ToLatLon.from_cc_name),
    ...
    def __init__(self):
        ...
        gui.appendRadioButton(
            modes, '将区域名称编码为地理坐标:', insertInto=top)
        ...
        gui.appendRadioButton(
            modes, '将经纬度反向地理编码为区域:', insertInto=top)
        box = gui.indentedBox(top)
        model = DomainModel(parent=self, valid_types=ContinuousVariable)
        self.domainmodels.append(model)
        combo = gui.comboBox(
            box, self, 'lat_attr', label='维度:', orientation=Qt.Horizontal,
            callback=lambda: self.commit(), sendSelectedValue=True, model=model)
        combo = gui.comboBox(
            box, self, 'lon_attr', label='经度:', orientation=Qt.Horizontal,
            callback=lambda: self.commit(), sendSelectedValue=True, model=model)
        gui.comboBox(
            box, self, 'admin', label='行政区级别:', orientation=Qt.Horizontal,
        ...
        gui.checkBox(
            ...
            label='使用其他区域属性扩展编码数据',
            callback=lambda: self.commit(),
            toolTip='使用区域属性扩展编码数据，例如 ISO 代码、大洲、子区域、区域类型、经济类型、FIPS/HASC 代码等。')
