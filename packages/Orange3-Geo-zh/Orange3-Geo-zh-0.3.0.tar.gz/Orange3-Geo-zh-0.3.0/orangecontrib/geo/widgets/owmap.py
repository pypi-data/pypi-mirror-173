...
class OWMap(OWDataProjectionWidget):
    ...
    def _add_controls(self):
        ...
        lat_lon_box = gui.vBox(self.controlArea, box="布局", spacing=0)
        options = dict(
            labelWidth=75, orientation=Qt.Horizontal, sendSelectedValue=True,
            valueType=str, contentsLength=14
        )

        gui.comboBox(lat_lon_box, self, 'graph.tile_provider_key', label='地图:',
                     items=list(TILE_PROVIDERS),
                     callback=self.update_tile_provider, **options)

        gui.comboBox(lat_lon_box, self, 'attr_lat', label='纬度:',
