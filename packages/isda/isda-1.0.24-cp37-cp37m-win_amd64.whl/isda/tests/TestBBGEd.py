...
class MyTestCase(unittest.TestCase):
    ...
    def test_compute_isda_upfront_back_to_spread(self):
        ...
        wall_time_list = list()
        recovery_rate = 0.3
        upfront_charge_clean = 0.09964817116299997 # backed out from (100 - clean price) / 100
        upfront_charge_dirty = 0.09673150449634324 # backed out from (dirty settlement amount / 1e6)
        ...
        f = calculate_spread_from_upfront_charge(self.trade_date,
                                                 ...
                                                 self.swap_rates,
                                                 self.swap_tenors,
                                                 upfront_charge_clean if is_upfront_clean else upfront_charge_dirty,
                                                 self.is_rofr,
                                                 is_upfront_clean,
        ...
        upfront_spread, status, duration_in_milliseconds = f

        wall_time_list.append(float(duration_in_milliseconds))
        print(f"recovery_rate {recovery_rate:,.2f} "
              f"upfront_spread {(upfront_spread * 10000) :,.6f} ")

        a = [wall_time_list]
        print("average execution {0}".format(average(a)))

        #
        # spread back to upfront
        #
        ...
        print(f"credit_spread {upfront_spread * 10000.:,.2f} "
