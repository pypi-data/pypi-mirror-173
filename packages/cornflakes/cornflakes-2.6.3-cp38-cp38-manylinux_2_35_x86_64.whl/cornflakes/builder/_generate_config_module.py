...
from cornflakes.decorator.config import config_group, is_config, INI_LOADER
...
def generate_group_module(
    ...
    declaration = [(f"{cfg_name}: List[{cfg[0].__class__.__name__}] = field(default_factory={cfg.__class__.__name__})"
                    if isinstance(cfg, list) else f"{cfg_name}: {cfg.__class__.__name__} = {cfg.__class__.__name__}()")
