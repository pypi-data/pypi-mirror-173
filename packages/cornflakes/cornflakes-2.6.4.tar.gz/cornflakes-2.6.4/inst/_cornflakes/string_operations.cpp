...
py::object eval_type(std::string value) {
  ...
  // is hex char
  if(value[0] == HEX_CHAR[0] && std::toupper(value[1]) == HEX_CHAR[1] && std::regex_match(value, hex_regex)){
  ...
  if (char_size < 6 && (upper_first_char == TRUE_CHAR ||
          upper_first_char == FALSE_CHAR)) {
