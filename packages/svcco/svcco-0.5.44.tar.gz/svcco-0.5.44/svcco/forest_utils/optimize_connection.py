...
def close_exact(data,point):
    ...
    #decision = np.zeros((data.shape[0]))
...
def opt(P1,P2,P3,P4,R,collision_vessels): #P1,P2,P3,P4,R):
    ...
    def get_spline(data,P2=P2,P4=P4,V1=V1,V2=V2,CTR0=CTR0,CTR1=CTR1,CTR2=CTR2,CTR3=CTR3,MID=MID,LENGTH=LENGTH):
        ...
        try:
            tck1, u = interpolate.splprep([SPT1[:,0],SPT1[:,1],SPT1[:,2]],s=0,k=2)
        except:
            CTR1 = CTR1 - data[0:3]
            CTR2 = CTR2 - data[3:]
    ...
    #init[0:3] = CTR1
    #init[3:]  = CTR2
    def cost(data,grad,spline_gen=get_spline,angle_threshold=angle_threshold,R=R,P1=P1,CTR4=CTR4,collision_vessels=collision_vessels):
        ...
        a_try = np.sum(angles)
        return collisions+a_sum*spline_length #a_sum+spline_length + collisions#+np.tanh(num)
    ...
    def get_curvature(data):
        ...
        return curve
    def cost2(data,R=R,get_spline=get_spline):
        spf,spdf,spddf = get_spline(data)
        tt = np.linspace(0,1,num=500)
        pts = spf(tt).T
        spline_length = np.sum(np.linalg.norm(np.diff(pts,axis=0),axis=1))
    ...
    def cost3(t_vector,grad,spline=None,angle_threshold=angle_threshold,P1=P1,P2=P2,P3=P3,MID=MID,CTR4=CTR4):
        ...
        sample_pts = np.vstack((P1,sample_pts,P3))
        vec = np.diff(sample_pts,axis=0)
        vec = vec/np.linalg.norm(vec,axis=0)
        angles = []
        for i in range(vec.shape[0]-1):
        ...
        return a_sum
    return get_spline,show,cost,init,LENGTH,check,get_points,get_curvature,cost3,collision_vessels,cost_polish
