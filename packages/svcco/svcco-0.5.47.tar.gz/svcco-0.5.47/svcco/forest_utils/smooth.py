...
def link(forest,angle_threshold=120):
    all_connections = []
    for network in range(forest.number_of_networks):
        ...
        for tree in range(forest.trees_per_network[network]):
            ...
            tree_connections = []
            for edge in range(len(assignment)):
                R        = forest.networks[network][tree].data[assignment[edge],21]
                V1,V2,P4 = get_initial_vectors(forest,network,assignment,tree,edge)
                angle    = get_angle(V1,V2)
                success  = False
                temp_end = P4
                while not success:
                    if angle > angle_threshold:
                        # check new vessel for collision
                        if not collision:
                            # append vessel to link list
                            if np.all(temp_end==P4):
                                # if the new vessel reaches the connection
                                # endpoint then stop iterating on vessel
                                # creation
                                success = True
