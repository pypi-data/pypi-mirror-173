...
def main():
    ...
    if config_file_location is None:
        config_location = os.path.join(data_dict["Location"], "oknserver_graph_plot_config.json")
        config_build_in_exist = os.path.isdir(config_location)
        ...
        if not config_dir_exist:
            print(f"Config location input:{directory_input} does not exist.")
            config_location = os.path.join(data_dict["Location"], "oknserver_graph_plot_config.json")
