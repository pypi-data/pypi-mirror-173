...
def get_plot_info(data_dir, x_data_header, y_data_header, x_label_input, y_label_input,
                  signal_csv_folder_name_input, signal_csv_name_input, sp_column_name_input,
                  sp_line_color_input, sp_line_thickness_input, qp_column_name_input, qp_line_color_input,
    ...
    for folder_name in folder_array:
        ...
        sp_array, qp_array = get_sp_and_qp_array(signal_csv_dir, sp_column_name_input, qp_column_name_input, y_array)
...
def plot_combined_graph(plot_info_input, max_graph_in_a_row_input, output_dir, x_axis_limit_input, y_axis_limit_input,
                        adjust_limit_boolean, adjust_limit_info_input, line_color, line_thickness, image_scale_input,
                        output_image_name_input):
    # print(adjust_limit_boolean)
    # print(adjust_limit_info_input)
    if adjust_limit_boolean:
        ...
        x_lower_limit = x_axis_limit_input[0]
        x_upper_limit = x_axis_limit_input[1]
        y_lower_limit = y_axis_limit_input[0]
        y_upper_limit = y_axis_limit_input[1]
...
def main():
    ...
    parser.add_argument("-t", dest="plot_type", required=True, default=sys.stdin,
                        help="the type of plot", metavar="plot type")
    ...
    config_to_be_read = "oknserver_graph_plot_config.json"
    ...
    if not dir_exist:
        ...
        if config_file_location is not None:
            config_dir_exist = os.path.isdir(config_file_location)
            if not config_dir_exist:
                f"Config location input:{directory_input} does not exist"
            else:
                config_to_be_read = config_file_location

    # Opening oknserver graph plot config
    with open(config_to_be_read) as f:
        ...
        auto_adjustment = summary_plot_info["auto_axis_adjustment"]
        ...
        if plot_config_info is not None and dir_exist:
            if type_input == "trial":
                ...
                signal_csv_dir = os.path.join(directory_input, "result", "signal.csv")
                ...
                plot_combined_graph(plot_data_array, max_graph_in_a_row, directory_input, x_axis_limit, y_axis_limit,
                                    auto_adjustment, auto_adjust_info, graph_line_color, graph_line_thickness,
