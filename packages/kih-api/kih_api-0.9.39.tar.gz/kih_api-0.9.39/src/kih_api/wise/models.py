...
class WiseAccount:
    ...
    def get_reserve_account(self, currency: global_common.Currency, reserve_account_name: str) -> ReserveAccount:
        return typing.cast(ReserveAccount, list(filter(lambda account: account.currency == currency and isinstance(account, ReserveAccount) and account.name == reserve_account_name,self.account_list))[0])
...
class Account(ABC):
    ...
    def transfer(self, api_key: str, recipient: Recipient, receiving_amount: Decimal, to_currency: global_common.Currency, reference: str) -> Transfer:
        transfer: Transfer = Transfer.execute(api_key, receiving_amount, recipient, self.currency, to_currency, reference, self.user_profile.type)

        if transfer.is_successful:
            if isinstance(self, CashAccount):
                self.balance = Account.get_by_profile_type_and_currency(api_key, self.user_profile.type, self.currency).balance
            else:
                self.balance = ReserveAccount.get_reserve_account_by_profile_type_currency_and_name(api_key, self.user_profile.type, self.currency, self.name, False).balance

        return transfer

    def intra_account_transfer(self, api_key: str, to_account: Union[CashAccount | ReserveAccount], receiving_amount: Decimal) -> IntraAccountTransfer:
        intra_account_transfer: IntraAccountTransfer = IntraAccountTransfer.execute(api_key, receiving_amount, self, to_account, self.user_profile.type)    # type: ignore[arg-type]
