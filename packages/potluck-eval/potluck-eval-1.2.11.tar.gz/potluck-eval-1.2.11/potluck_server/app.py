...
def route_dash(
    ...
    # Grab latest-outcomes info for exercises
    outcomes = storage.fetch_best_outcomes(course, semester, effective_user)
...
def route_deliver(course, semester):
    ...
    for author in authors:
        ...
        on_time = phase != "due"
        status, ecredit, gcredit = exercise_credit(
            einfo,
            on_time,
            outcomes
        )
...
def route_exercise(
    ...
    # Get outcome info for this exercise
    outcomes = storage.fetch_best_outcomes(course, semester, target_user)
...
def route_ex_gradesheet(
    ...
    for stid in sorted(
        ...
        # Get exercise outcomes for this student
        row_outcomes = storage.fetch_best_outcomes(course, semester, stid)
...
def exercise_credit(exercise_info, on_time, outcomes):
    """
    ...
    individual exercise dictionary). The `on_time` value should be a
    boolean indicating whether or not the exercise outcomes were
    submitted on-time. The `on_time` value only affects the group
    credit, not the exercise credit, according to the configuration's
    `LATE_EXERCISE_CREDIT_FRACTION` value when it's false.
    ...
    # Modify group credit if this is late
    if not on_time:
        group_credit *= app.config.get("LATE_EXERCISE_CREDIT_FRACTION", 0.5)
