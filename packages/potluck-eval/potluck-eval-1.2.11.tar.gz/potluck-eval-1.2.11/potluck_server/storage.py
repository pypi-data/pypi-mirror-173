...
def exercise_key(course, semester, username, exercise):
    """
...
def best_exercises_key(course, semester, username):
    """
    The Redis key for the per-exercise-best-outcomes-list data for a
    particular user in a course/semester.
    """
    return redis_key(
        ':'.join(
            [course, semester, "exercises", username]
        )
    )
...
def fetch_outcomes(course, semester, username, exercise):
    """
    Fetches the outcomes-list information for the given
    user/course/semester/exercise. The result will be a list of
    dictionaries each representing a single submission, in chronological
    order. Each will have the following keys:
    ...
    - "credit": A number indicating how much credit (0-1) was earned for
        this outcome. Note that the best-credit outcome is retained in
        the best-outcomes dictionary, with ties broken towards
        more-recent submissions.

    Returns None instead of a list if there is no information for that
    user/exercise yet, or if an error is encountered while trying to
    access that information.
    """
    # Redis key to use
    key = exercise_key(course, semester, username, exercise)
...
def fetch_best_outcomes(course, semester, username):
    """
    ...
    - "credit" - A number indicating how much credit this exercise is
        ...
        with the submission (like wrong # of outcomes).

    Only the IDs of exercises the user has submitted at least once will
    appear in the base dictionary.

    Returns None if an error is encountered, but otherwise returns at
    least an empty dictionary, including when the specified user hasn't
    submitted anything yet.
    """
    # Redis key to use
    key = best_exercises_key(course, semester, username)

    try:
        exists = _REDIS.exists(key)
    except Exception:
        flask.flash("Error checking for best-outcomes info.")
        tb = potluck.html_tools.string_traceback()
        print(
            "Failed to check for best-outcomes info at '{}':\n{}".format(
                key,
                tb
            ),
            file=sys.stderr
    ...
    # Return an empty dictionary without making a fuss if the key just
    # doesn't exist
    if not exists:
        return {}

    try:
        allJSON = _REDIS.hgetall(key)
        result = {key: json.loads(allJSON[key]) for key in allJSON}
    except Exception:
        flask.flash("Error fetching or decoding best-outcomes info.")
        tb = potluck.html_tools.string_traceback()
        print(
            "Failed to fetch best-outcomes info at '{}':\n{}".format(
                key,
                tb
            ),
            file=sys.stderr
...
def save_outcomes(
    ...
    # Also store info in best-outcomes map if it is good
    best_key = best_exercises_key(course, semester, username)
    # TODO: We should pipeline this with the other write to try to more
    # completely avoid situations where the first succeeds and the second
    # fails!
    try:
