...
class ProcessedImage(Image):
    ...
    def extract_colourchecker_rgb_patches(self, checker_name, corners, size_rect):
        ...
        if pte.is_colourchecker_implemented(checker_name):
            ...
            self.patch_size[checker_name] = {}
            self.patch_size[checker_name]["size_rect"] = size_rect
...
class RawImage(Image):
    ...
    __illuminant = None
    __observer = None 
    __colourchecker_RGB = None # dict objects only property, not setter
    __patch_size = None
    __rgb_data = None
    ...
    def extract_colourchecker_rgb_patches(self, checker_name, corners, size_rect):
        ...
        if pte.is_colourchecker_implemented(checker_name):
            ...
            # update            
            current_colourchecker_rgb = self.__colourchecker_RGB.keys()
            if checker_name not in current_colourchecker_rgb:
                # create new
                colourchecker = ColourCheckerRGB(checker_name=checker_name, illuminant=self.illuminant, observer=self.observer, data=rgb_patches, metadata=self.metadata)
                # update
                self.__colourchecker_RGB[checker_name] = {}
                self.__colourchecker_RGB[checker_name]["data"] = colourchecker
                self.__colourchecker_RGB[checker_name]["draw"] = [checker_coordinates]
            else:
                # update
                self.__colourchecker_RGB[checker_name]["data"].update_patches(rgb_patches)
                current_draw = self.__colourchecker_RGB[checker_name]["draw"]
                current_draw.append(checker_coordinates)
        else:
            raise ColourCheckerError("ColourChecker not implemented")
    ...
    def get_ColourCheckerRGB(self, checker_name):
        ...
        '''
        if checker_name in self.__colourchecker_RGB.keys():
            return self.__colourchecker_RGB[checker_name]["data"]
        else:
            raise ColourCheckerError(f"ColourChecker not present on image: {self.__colourchecker_RGB.keys()}")
    ...
    def get_patch_from_colourchecker(self, checker_name, patch_id):
        ...
        if checker_name in self.__colourchecker_RGB.keys():
            if patch_id in self.__colourchecker_RGB[checker_name]["data"].patches.keys():
                r,g,b = self.__colourchecker_RGB[checker_name]["data"].get_patch_data(patch_id)
                return r,g,b
            else:
                raise PatchError(f"Patch id not present in the ColourChecker {checker_name}")
        else:
    ...
    def show_colourchecker(self, checker_name, parameters_draw=None, show_image = True, method ="OpenCV", save_image = False, output_path = None, bits=16):
        ...
        '''
        # get coordinates
        coordinates_list = self.__colourchecker_RGB[checker_name]["draw"]
        rgb_data_with_patches = self.rgb_data.copy()
        parameters_draw = {"size_rect":self.__patch_size[checker_name]["size_rect"]} if parameters_draw==None else parameters_draw # update patch size
