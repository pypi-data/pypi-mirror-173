...
def scan_folder(folder_name="", glob_folder_name=".", recursive=True):
    ...
    """
    if recursive:
        return pathlib.Path(folder_name).rglob(glob_folder_name)
    else:
        return pathlib.Path(folder_name).glob(glob_folder_name)
...
def scan_file(folder_name="", glob_filename="*.*", recursive=True):
    ...
    """
    if recursive:
        return pathlib.Path(folder_name).rglob(glob_filename)
    else:
        return pathlib.Path(folder_name).glob(glob_filename)
