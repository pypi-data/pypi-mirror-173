...
def data2vectordb(total_result=None):
  upsert_api = "http://52.79.176.35:8000/upsert/"
  if total_result is None:
    response = requests.get(upsert_api)
  else:
    response = requests.post(upsert_api, data=json.dumps({"result": total_result}), headers={"Content-Type": "application/json"},)
  return "Done"

def search_func(query, top_k, debug=False):
