...
class CreateV1AllKeypointsFile(modalities.ModalityPreProcess):
    ...
    Those two files have to be consolidated into a single json since the SDK assumes
    that the content of any given modality will always appear in a single json file.
    We consolidate both files into a single one, located at '<DATAPOINT_CAMERA_PATH>/key_points/all_key_points.json'
    to match v2 keypoints\\' files layout.
    """
