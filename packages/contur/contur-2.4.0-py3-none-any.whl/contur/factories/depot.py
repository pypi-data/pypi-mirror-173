...
class Depot(object):
    ...
    def add_point(self, yodafile, param_dict):
        ...
        self._point_list.append(LikelihoodPoint(yodaFactory=yFact, paramPoint=param_dict))
        ...
        try:
            ...
            for stat_type in cfg.stat_types:
                frame['CL{}'.format(stat_type)] = [
                    likelihood_point.yoda_factory.get_full_likelihood(stat_type).getCLs() for likelihood_point in self.points]

                if include_dominant_pools:
                    frame['dominant-pool{}'.format(stat_type)] = [
                        likelihood_point.yoda_factory.get_dominant_pool(stat_type).pools
                        for likelihood_point in self.points
                    ]
                    frame['dominant-pool-tag{}'.format(stat_type)] = [
                        likelihood_point.yoda_factory.get_dominant_pool(stat_type).tags
                        for likelihood_point in self.points
                    ]
                ...
                if include_per_pool_cls:
                    poolsDict = {}
                    for likelihood_point in self.points:
                       for block in likelihood_point.yoda_factory.get_sorted_likelihood_blocks(stat_type):
...
#            cfg.contur_log.error("List of LikelihoodPoints is empty, add parameter points to depot")
    ...
    def write_summary_file(self, message):
        ...
        if cfg.gridMode:
            ...
            yfactory = self.points[0].yoda_factory
    ...
    def write_summary_dict(self, output_opts):
        ...
        for stat_type in cfg.stat_types:
            ...
            full_like = self.points[0].yoda_factory.get_full_likelihood(stat_type)
            like_blocks = self.points[0].yoda_factory.get_sorted_likelihood_blocks(stat_type)
...
class LikelihoodPoint(object):
    """
    Save the plotting infomation about likelihood points in a run, including a full likelihood result, 
    exclusions result, test b result, test s+b result with related stat_type and a paramter point dictionary
    """
    ...
    def __init__(self, paramPoint={}, yodaFactory=None):
        ...
        if yodaFactory is not None:
            for stat_type in cfg.stat_types:
                ...
                try:
                    self.combined_exclusion_dict[stat_type] = yodaFactory.get_full_likelihood(stat_type).getCLs()
                    for p in yodaFactory.get_sorted_likelihood_blocks(stat_type):
