...
def write_grid_data(args, conturDepot):
    ...
    for likelihood_point in conturDepot.points:
        ...
        for stat_type in cfg.stat_types:   
            ...
            if likelihood_point.yoda_factory.get_full_likelihood(stat_type).getCLs() is not None:
                ...
                c.execute("insert into run (id,map_id,model_point_id,events_num,stat_type,combined_exclusion,addition_argument) \
                    values (?,?,?,?,?,?,? )",
                        (None, map_id, model_point_id, events_num, stat_type, likelihood_point.yoda_factory.get_full_likelihood(stat_type).getCLs(),addition_argument))

                run_id = c.execute("select id from run order by id desc;").fetchone()[0]

                for x in likelihood_point.yoda_factory.get_sorted_likelihood_blocks(stat_type):
