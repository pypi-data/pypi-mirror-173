...
from metaloop.exception import ResourceNotExistError, InvalidParamsError
...
class Dataset(DatasetMeta):
    ...
    def import_data(
        ...
        if storage_type == "cos":
            if not os.path.isdir(file_path):
                ...
                upload_id = self._upload_file(storage_type, file_path)
            else:
    ...
    def _download_file(
        ...
        index = kwargs["url"].strip("/").index("/")
        key = kwargs["url"][index + 2:]
        ...
        return kwargs["identifier"], Item(kwargs["bucket"], key, file_name, image_path if image_path else key)
    ...
    def _download_http_file(
        ...
        return Item("", url, file_name, url[index + 8:])
