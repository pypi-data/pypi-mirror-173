    ...
    def _get_value(self, path: str):
        ...
        for k in path:
            if isinstance(v, list) or isinstance(v, tuple):
                k = int(k)
                if len(v) < k:
                    raise KeyError(f'array out of bound (index = {k}) when retrieving value where path is {path}')
            else:
                if k not in v:
                    raise KeyError(f'key ({k}) not exist when retrieving value where path is {path}')
            v = v[k]
        return v
