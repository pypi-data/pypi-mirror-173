...
def update_infodb(valor_atualizar, just_append=False, path_dir_=None, antiblock=False):
    global editando
    if client_use:
        ...
        if path_dir_ == None:
            ...
            path_dir = path_dir_
        testa_existe_info = dirfdb(antiblock=True)
        if just_append == False:
            ...
            contador_exect = 1
            dict_pushed = dirfdb(antiblock=True)
            if dict_pushed == None:
...
def dirfdb(searchdb=None, antiblock=False, path_dir_=None):
    global editando
    if client_use:
...
def remove_infodb(path_delete=None, path_dir_=None, antiblock=False):
    ...
    if client_use:
        ...
        global contador_exec
        db_delete = dirfdb(antiblock=True)
        if db_delete == None:
            pass
        else:
...
def dirfdbp():
    tempor = dirfdb()
    if tempor != None:
        print(dumps(dirfdb(), indent=4, ensure_ascii=False).encode('utf-8').decode())
    else:
        print(None)
