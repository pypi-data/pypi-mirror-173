use crate::circuit::{CircuitConstructionError, Einsum, EinsumAxes, Symbol};
...
impl EinsumSpec {
            ...
            let output_ints = output
                ...
                .map(|x| {
                    ALPHABET_INV
                        .get(&x.to_string())
                        .cloned()
                        .ok_or(CircuitConstructionError::EinsumStringInvalid {
                            string: string.clone(),
    ...
    pub fn fancy_string_to_ints(string: String) -> (Vec<EinsumAxes>, EinsumAxes) {
        if let [inputs, output] = string.split("->").collect::<Vec<&str>>()[..] {
            let mut string_to_int: HashMap<&str, u8> = HashMap::new();
            let input_ints = inputs
                .split(',')
                .map(|string| {
                    string
