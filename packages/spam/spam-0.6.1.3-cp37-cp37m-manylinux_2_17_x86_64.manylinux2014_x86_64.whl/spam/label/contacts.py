...
def contactOrientations(volBin, volLab, watershed="ITK", peakDistance=1, markerShrink=True, verbose=False):
    ...
    if watershed == "RW":
        ...
        #Calculate local Max
        localMaxiPointsA = skimage.feature.peak_local_max(distanceMapA, min_distance=peakDistance, exclude_border=False, labels=subVolBinA)
        localMaxiPointsB = skimage.feature.peak_local_max(distanceMapB, min_distance=peakDistance, exclude_border=False, labels=subVolBinB)
...
def _markerCorrection(markers, numMarkers, distanceMap, volBin, peakDistance=5, struc=numpy.ones((3,3,3))):
    ...
    if numpy.amax(markers) > 2:
        while numpy.amax(markers) > 2 and peakDistance < min(volBin.shape):
            ...
            localMaxiPoints = skimage.feature.peak_local_max(distanceMap, min_distance=peakDistance, exclude_border = False, labels=volBin, num_peaks=2)
