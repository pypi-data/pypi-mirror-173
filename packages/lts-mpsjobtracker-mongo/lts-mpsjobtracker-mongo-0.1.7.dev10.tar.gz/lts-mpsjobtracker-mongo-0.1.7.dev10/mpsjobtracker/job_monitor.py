...
class JobMonitor():
    ...
    def check_inprocess_jobs(self):
        ...
        for job_doc in inprocess_jobs:
            check_time = True
            print(f"inprocess job {str(job_doc['_id'])}", flush=True)
            parent_ref = job_doc.get('parent_ref')
            if (parent_ref is not None):
                # Check if this job is complete by seeing if all child jobs are failed or successful
                print("Attempting to resolve job " + str(parent_ref), flush=True)
                child_jobs = job_tracker.get_child_jobs(parent_ref)
                job_finished = True
                for child_job in child_jobs:
                    if child_job['job_management']['job_status'] not in ['failed', 'success']:
                        job_finished = False
                if job_finished and len(child_jobs) > 0:
                    print("Found and a complete job...", flush=True)
                    job_tracker.set_job_status('success', parent_ref)
    ...
    def handle_stalled_job(self, job_doc):
        ...
        if (number_of_tries < max_number_of_tries):
            #Requeue
            print("Requeued job id" + job_doc["_id"])
