    ...
    def check_inprocess_jobs(self):
        ...
        for job_doc in inprocess_jobs:
            ...
            if (parent_ref is not None):
                ...
                print(str(datetime.datetime.utcnow().replace(tzinfo=datetime.timezone.utc, microsecond=0)), flush=True)
                ...
                if job_finished and child_job_count > 0:
                    print("Found a complete job...", flush=True)
                    ...
                    check_time = False
                    celeryapp.execute.send_task("tasks.tasks.do_task", args=[{'drsId': str(job_id), 'status': 'success'}], kwargs={}, queue=_success_queue)
                else:
