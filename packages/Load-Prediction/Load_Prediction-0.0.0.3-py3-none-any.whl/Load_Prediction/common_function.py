...
class Common:
    ...
    def real_load_days(self, air_condition, predicttiondate, weekday_yn):
        ...
        sql = " SELECT count(DISTINCT(r.RealDate)) \
                 ...
                 WHERE r.SiteCode = '" + Common.sitecode + "' \
                   AND isnull(p.CoolHeatingIndication, '0') = '" + air_condition + "' \
    ...
    def predict_load_days(self, air_condition, predicttiondate, weekday_yn):
        ...
        sql = "SELECT COUNT(DISTINCT(PredictionDate)) \
                ...
                WHERE SiteCode = '" + Common.sitecode + "' \
                  AND isnull(CoolHeatingIndication, '0') = '" + air_condition + "' \
    ...
    # 예측부하 테이블에서 냉반방구분 확인
    # noinspection PyMethodMayBeStatic
    def search_coolheatingindication(self, predictiondate):
        db = Common.conn()
        cursor = db.cursor()
        sql = "SELECT top 1 isnull(max(p.CoolHeatingIndication),0) \
    ...
    def determine_air_condition(self, predictiondate):
        ...
        this_month = int(predictiondate[4:6])
        air_condition = Common.search_coolheatingindication(Common.sitecode, predictiondate)
    ...
    def load_by_predict_foretime(self, predictiondate, air_condition):
        ...
        sql = " SELECT z.BuildingNo, z.ZoneNo, \
                ...
                SELECT p.BuildingNo, p.ZoneNo, p.PREDICTIONDATE AS Date, p.PREDICTIONHOUR AS Hour, \
                  ...
                  FROM T_LOAD_PREDICTION_INFO p \
                 WHERE p.SiteCode = '" + Common.sitecode + "' \
                   AND isnull(p.CoolHeatingIndication, '0') = '" + str(air_condition) + "' \
                   AND p.PREDICTIONDATE between \
                         format(CAST(dateadd(year,-1, '" + predictiondate + "') AS date), 'yyyy0101') \
                         AND convert(varchar(10),dateadd(d, -1, '" + predictiondate + "'), 121) "
        df = pd.read_sql(sql, db)
    ...
    def insert_loadpredictioninfo(self, df_row, coolheating_indication, dt_hm):
        db = Common.conn()
        cursor = db.cursor()
        sql = " SELECT COUNT(*) FROM T_LOAD_PREDICTION_INFO \
                 WHERE SITECODE = '" + Common.sitecode + "' \
        ...
        if count < 1:
            sql = "INSERT INTO T_LOAD_PREDICTION_INFO(SITECODE, BUILDINGNO, ZONENO, PREDICTIONDATE, \
                          ...
                          NIGHTPOWERUNITRATE, GASUNITRATE, DHUNITRATE, COOLHEATINGINDICATION) \
            ...
            #        df_row.RevisionPredictLoad, 0, 0, CoolHeatingIndication)
            cursor.execute(sql, val)
            db.commit()

        else:
            # print("dt_hm:", dt_hm, "df_row.Hour:", df_row.Hour)
            if dt_hm < df_row.Hour:
                sql = " UPDATE T_LOAD_PREDICTION_INFO \
                           SET PredictionDiffTemp = %d \
                              ,PredictionDiffEnthalpy = %d \
                              ,PredictionSolarRadQ = %d \
                              ,PredictionLoad = %d \
                              ,PowerUnitRate = %d \
                              ,NightPowerUnitRate = %d \
                              ,GasUnitRate = %d \
                              ,DHUnitRate = %d \
                         WHERE SITECODE = '" + Common.sitecode + "' \
                           AND BuildingNo = '" + df_row.BuildingNo + "' \
                           ...
                           AND PREDICTIONDATE = '" + df_row.Date + "' \
                           AND PREDICTIONHOUR = '" + df_row.Hour + "' "
                val = (df_row.TempDiff, df_row.EnthalpyDiff, df_row.SOLARRADQ, df_row.PredictLoad,
                       round(df_row.PowerUnitRate * df_row.PredictLoad / 860, 2),
                       round(df_row.NightPowerUnitRate * df_row.PredictLoad / 860, 2),
                       round(df_row.GasUnitRate * df_row.PredictLoad * 4.1868 / 1000, 2),
                       round(df_row.DHUnitRate * df_row.PredictLoad / 1000, 2))
                cursor.execute(sql, val)
                db.commit()
        cursor.close()
        db.close()
                               ...
                               ELSE (CASE WHEN \
                                          ...
                                          ELSE ((CASE WHEN (S.PredictionSolarRadLoad is NULL) THEN 0 \
                                                      ELSE S.PredictionSolarRadLoad END) \
                                                         ...
                                                         + H.PredictionHeatTransLoad) * -1.0 \
                               END ) \
                       END) * (CASE WHEN ((SELECT T.PowerUnitRate FROM T_ENERGY_UNIT_INFO T \
                                            WHERE T.SITECODE = A.SITECODE AND 8 = T.Month) IS NULL) THEN 0 \
                                    ELSE (SELECT T.PowerUnitRate FROM T_ENERGY_UNIT_INFO T \
                       ...
                       /*-- 난방이 - 면 + 로 변경 */ \
                                          ELSE ((CASE WHEN (S.PredictionSolarRadLoad is NULL) THEN 0 \
                                                      ELSE S.PredictionSolarRadLoad END) \
                                                          ...
                                                          + H.PredictionHeatTransLoad) * -1.0 \
                                      END) \
                            END) * (CASE WHEN ((SELECT T.NightPowerUnitRate FROM T_ENERGY_UNIT_INFO T \
                                                 WHERE T.SITECODE = A.SITECODE AND  8 = T.Month) IS NULL) THEN 0 \
                                         ELSE (SELECT T.NightPowerUnitRate FROM T_ENERGY_UNIT_INFO T \
                                                WHERE T.SITECODE = A.SITECODE AND 8 = T.Month) / 860.0 \
                                    END) \
                      , 0) AS NightPowerUnitRate, \
                     /*--도시가스요금 */ \
                       ...
                       /*-- 난방이 - 면 + 로 변경 */ \
                                     ELSE ((CASE WHEN (S.PredictionSolarRadLoad is NULL) THEN 0 \
                                                 ELSE S.PredictionSolarRadLoad END) \
                                                      ...
                                                      + H.PredictionHeatTransLoad) * -1.0 \
                               END) \
                       END) * (CASE WHEN ((SELECT T.GasUnitRate FROM T_ENERGY_UNIT_INFO T \
                                            WHERE T.SITECODE = A.SITECODE AND 8 = T.Month) IS NULL) THEN 0 \
                                    ELSE (SELECT T.GasUnitRate FROM T_ENERGY_UNIT_INFO T \
                       ...
                       /*-- 난방이 - 면 + 로 변경 */ \
                                          ELSE ((CASE WHEN (S.PredictionSolarRadLoad is NULL) THEN 0 \
                                                      ELSE S.PredictionSolarRadLoad END) \
                                                           ...
                                                           + H.PredictionHeatTransLoad) * -1.0 \
                                          END) \
                     END) * (CASE WHEN ((SELECT T.DHUnitRate FROM T_ENERGY_UNIT_INFO T \
                                          WHERE T.SITECODE = A.SITECODE AND 8 = T.Month) IS NULL) THEN 0 \
                                  ELSE (SELECT T.DHUnitRate FROM T_ENERGY_UNIT_INFO T \
                                         WHERE T.SITECODE = A.SITECODE AND 8 = T.Month) / 1000.0 \
                             END) \
                     , 0) AS DHUnitRate, \
                   '" + str(air_condition) + "' AS CoolHeatingIndication, \
                     /*--타임스탬프 */ \
...
real_date = c.revision_load_date('20180805', '1')
print("real_date:", real_date)

aircondition = c.determine_air_condition('20180728')
print("aircondition:", aircondition)

df_m1 = c.building_features('20180728', aircondition)
print("df_m1:", df_m1)

coldstartdate, heatingstartdate = c.operating_start_date()
print("coldstartdate :", coldstartdate, "heatingstartdate:", heatingstartdate)

df_days = c.op_days('20160406', '20160531')
print("df_days:", df_days)

startmonth, endmonth = c.cooling_month()
print("startmonth, endmonth:", startmonth, endmonth)

coldbasetemp, heatingbasetemp = c.base_temp()
print("coldbasetemp :", coldbasetemp, "heatingbasetemp:", heatingbasetemp)

avg_temp = c.avg_temp('20160111')
print("avg_temp:", avg_temp)


load_prediction_method = c.load_prediction_method()
print("load_prediction_method:", load_prediction_method)

predict_load_days = c.predict_load_days('2', '20160111', '1')
print("predict_load_days:", predict_load_days)

real_load_days = c.real_load_days('2', '20160111', '1')
print("real_load_days:", real_load_days)

dt_week, holidaycode = c.is_weekday('20160111')
is_holiday = c.is_holiday('20160111')
print("dt_week:", dt_week, "holidaycode:", holidaycode, "is_holiday:", is_holiday)

coolheatingindication = Common.search_coolheatingindication(sitecode, '20180907')
print("coolheatingindication:", coolheatingindication)

season_changes = c.season_changes('20180905')
print("season_changes: [", season_changes, "]")

# pd.set_option('display.max_rows', None)
df_revision = c.revision_load('20180818')
print("df_revision:", df_revision)

loadcorrectionday, correctionhour = c.revision_days()
print("loadcorrectionday:", loadcorrectionday, "correctionhour", correctionhour)
"""

# df = c.load_by_real_foretime('20180808', '1')
# print("df:", df)
# df_hour = df[df['Hour']== '0900']
# df_load = df_hour['Load']
# outlier_index = c.get_outlier(df_hour, 'Load', 1.5)
# print("outlier_index:\n", outlier_index)


# df = c.load_by_real_foretime_realweather('20180905','1')
# print("df:", df)

# forecast_date = c.forecast_date()
# print("forecast_date:", forecast_date)

# df = Common.predict_load_model_learning(sitecode, 'M', '1', '2', '20180907')
# print("predict_load_model_learning df:", df)

# df = Common.real_load_model_learning(sitecode, 'M', '1', '2', '20180907')
# print("real_load_model_learning df:", df)

# setuptemp, setupenthalpy = c.zone_info('A', 11, '1')
# print("setuptemp:", setuptemp, "setupenthalpy", setupenthalpy)

# realtemp, realenthalpy = c.one_hour_weather_real_info('20180907', '0900')
# print("realtemp:", realtemp, "realenthalpy", realenthalpy)


# df = c.predict_load_by_foretime_predict_weather('20180808', aircondition)
# print(" predict_load_by_foretime_predict df :\n", df.head(10))

# holiday_cnt, holiday = c.holiday_data('20180907')
# print("holiday_cnt:", holiday_cnt, "\nholiday:", holiday)
