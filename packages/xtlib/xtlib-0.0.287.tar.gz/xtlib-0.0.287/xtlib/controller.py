...
class XTController(rpyc.Service):
    ...
    def start_local_run_core(self, run_info, cmd_parts, rundir, rundir_index):
        ...
        if context.mirror_dest and context.mirror_files and (context.grok_server or context.mirror_dest == "storage"):
            self.start_mirror_workers(store, run_info, rundir, run_name, context)
    ...
    def start_mirror_workers(self, store, run_info, rundir, run_name, context):
        log_info("starting MIRROR worker for run", run_name)
        mirror_files_list: list = context.mirror_files
        mirror_dest: str = context.mirror_dest

        log_info("mirror-dest", mirror_dest)
        log_info("mirror_files=", mirror_files_list)

        workers = []

        for mf_path in mirror_files_list:
            worker = MirrorWorker(store, rundir, mirror_dest, mf_path, context.grok_server, context.ws, run_name, 
