        ...
        try:
            if self.m3u8字符串:
                ...
                if 'content-type' in req_headers:
                    #m3u8 类型
                    if 'mpegurl' in req_headers['content-type'].lower():
                        pass
                    #其他类型
                    else:
                        # 返回文件长度
                        rt = 下载文件_进度条(文件路径=self.fpath, 网址=self.url, 线程数=self.线程数, 多线程=True, 分段长度=1 * 1024)
                        return rt
                req_m3u8 = self.get_url(self.url, type_='url')
                lines = req_m3u8.text  # 获取M3U8的文件内容
                self.m3u8字符串 = lines
            # 读取文件里的每一行
