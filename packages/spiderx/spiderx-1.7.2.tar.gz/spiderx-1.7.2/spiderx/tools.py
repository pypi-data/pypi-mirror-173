    ...
    def get_items(self):
        try:
            if self.m3u8字符串:
                ...
                lines = self.get_url(self.url,type_='url').text  # 获取M3U8的文件内容
                self.m3u8字符串 = lines
            # 读取文件里的每一行
                ...
                for index, line in enumerate(file_line):
                    if "EXTINF" in line:
                        ...
                        if self.TS地址回调函数:  # 如果设置了回调
                            ...
                            ts_url = self.TS地址回调函数(ts_url)
                        else:
                            #没有http头 如果是网址 则拼接
                            if 'http' not in ts_url and self.url:
                                ts_url = self.链接前缀 + "/" + ts_url
                        if self.打印TS链接:
                            print(ts_url)
                        self.items.append({'i':n, 'url':ts_url})
                        n += 1
                if unknow:
        ...
        for i in range(self.try_num):
            try:
                ...
                if i==self.try_num-1:
                    打印错误(e)
        return None
    def get_url(self, url,type_='url',try_num=None):
        try:
            if getattr(self.options_headers,type_):
                headers=self.headers
            else:
                headers=None
            return get_request(url, cookies=self.cookies, headers=headers, proxies=self.proxies, allow_redirects=True, verify=False,curl=self.curl, try_num=try_num if try_num else self.try_num,timeout=self.timeout)
        except BaseException as e:
