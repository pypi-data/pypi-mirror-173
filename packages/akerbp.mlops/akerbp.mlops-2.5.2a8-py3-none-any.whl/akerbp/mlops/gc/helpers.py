...
def create_service_account(service_account):
    ...
    try:
        subprocess.check_call(
            f"gcloud iam service-accounts list | grep -q {service_account}", shell=True
        )
        logging.debug(f"Found service account {service_account}")
    except subprocess.CalledProcessError:
        subprocess.check_call(
            f"gcloud iam service-accounts create {service_account}", shell=True
        )
...
def run_container(image_name, service_account):
    ...
    """
    logging.info(f"Run container, {image_name=}, {service_account=}, {env_vars=}")
...
def deploy_function(function_name, folder=".", **kwargs):
    """
