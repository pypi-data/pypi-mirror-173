...
class GtfoAutoBatchTableApp(GtfoTableApp):
    ...
    def _table_recovery_loop(self, checks=2, consume_multiplier=10):
        while self._pending_table_recoveries and checks:
            ...
            self.transaction._table_write()  # relay transaction's cached writes to the table's write cache
            self.check_table_commits(recovery_multiplier=consume_multiplier)
