...
class TableTransaction(Transaction):
    ...
    def _update_pending_table_writes(self, value):
        ...
        if self._is_not_changelog_message:
            ...
            self._pending_table_offset_increase[self.partition()] = 1 + self.offset() - self._table_offset(partition=self.partition())
    ...
    def _table_write(self):
        for p, msgs in self._pending_table_writes.items():
            if msgs:
                ...
                LOGGER.debug(f'Finalizing table entry batch write for table {table.table_name}')
                LOGGER.debug(f'Table offset before write: {table.offset}, expected after write: {self._table_offset(partition=p) + 1}')
                ...
                table.commit_and_cleanup_if_ready()
    ...
    def _table_read(self, partition=None):
        ...
        if partition is None:
            partition = self.partition()
        return self.app_tables[partition].read(self.key())
