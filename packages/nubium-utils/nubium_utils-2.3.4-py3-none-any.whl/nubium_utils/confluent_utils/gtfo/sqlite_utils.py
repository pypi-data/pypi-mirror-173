...
class SqliteGtfo:
    def __init__(self, table_name, table_path=None, auto_init=True, max_pending_writes_count=None, min_cache_count=None, max_cache_count=None):
        if not max_pending_writes_count:
            max_pending_writes_count = 1500
        if not min_cache_count:
            min_cache_count = 1000
        if not max_cache_count:
            max_cache_count = 1500
    ...
    def commit_and_cleanup_if_ready(self, recovery_multiplier=None):
        LOGGER.debug('Checking if table is ready to commit or prune cache...')
    ...
    def prune_db_cache(self):
        ...
        LOGGER.info(f'Pruning {remove_count} from {len(self.db_cache)} table db cached records')
