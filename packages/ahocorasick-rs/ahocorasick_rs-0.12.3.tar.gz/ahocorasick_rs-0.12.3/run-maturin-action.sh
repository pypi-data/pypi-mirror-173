...
curl -L https://github.com/PyO3/maturin/releases/download/v0.12.19/maturin-x86_64-unknown-linux-musl.tar.gz | tar -xz -C /usr/local/bin
maturin --version || true
which patchelf > /dev/null || python3 -m pip install patchelf
echo "::endgroup::"
maturin build --release --out target/wheels/ -i python3.7