	...
	real *fnorm_computation__)
{
    ...
    static doublereal rh, rm, econ_value__, dm1, dm2;
...
L350:
/*     write(*, 351), 'h = ', h */
/*     write(*, 351), 'savf = ', savf(1) */
/*     write(*, 351), 'yhi2 = ', yh(1, 2) */
/*     write(*, 351), 'acor = ', acor(1) */
/* 351  format(A10, e20.13) */
    i__2 = ls0001_1.n;
    for (i__ = 1; i__ <= i__2; ++i__) {
/* L360: */
...
/*     write (6, 364), y(1), y(2), y(3) */
/* 364  format('solver: sol = ', 3e13.6) */
...
/* ----------------------------------------------------------------------- */
L400:
/*     call econ (neq, tn, y, econ_value) */
/*     if (dabs(econ_value) .gt. 1e-5) go to 408 */
...
/* L408: */
    s_wsle(&io___27);
    do_lio(&c__9, &c__1, "econ crit failed at t=", (ftnlen)22);
    do_lio(&c__5, &c__1, (char *)&ls0001_1.tn, (ftnlen)sizeof(doublereal));
    do_lio(&c__9, &c__1, "for h=", (ftnlen)6);
    do_lio(&c__5, &c__1, (char *)&ls0001_1.h__, (ftnlen)sizeof(doublereal));
    do_lio(&c__9, &c__1, "econ is", (ftnlen)7);
    do_lio(&c__5, &c__1, (char *)&econ_value__, (ftnlen)sizeof(doublereal));
    e_wsle();
/* ----------------------------------------------------------------------- */
