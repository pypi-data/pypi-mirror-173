...
class CursorTracer(dbapi.CursorTracer):
    def _populate_span(
        self, span: trace_api.Span, cursor, *args: typing.Tuple[typing.Any, typing.Any]
    ):
        super()._populate_span(span, cursor, *args)
        if not span.is_recording():
            return

        token = os.environ.get("BASEGUARD_TOKEN")
        run = os.environ.get("BASEGUARD_RUN")
        if token and run:
            start = timeit.default_timer()
            span.set_attribute("db.callstack", traceback.extract_stack().format())
            inst_time = timeit.default_timer() - start
            span.set_attribute("db.backend_pid", cursor.connection.get_backend_pid())
            span.set_attribute("db.instrumentation_time", inst_time)
            span.set_attribute("baseguard.token", token)
            span.set_attribute("baseguard.run", run)
