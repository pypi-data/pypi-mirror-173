...
class S3Config(StorageConfig):
    ...
    def load_creds(cls, values: dict) -> dict:
        ...
        if REQUIRE_CREDS:
            creds_set = values["access_key_id"] and values["secret_access_key"]
            assert creds_set, "missing required s3:// creds"
        return values
...
class GSConfig(StorageConfig):
    ...
    def load_creds(cls, application_credentials: str | None) -> str | None:
        ...
        if REQUIRE_CREDS:
            assert creds, "missing required gs:// creds"
        ...
        if creds:
            ...
            assert isinstance(creds_obj, dict), "invalid gs:// creds (not a dict)"
            assert "type" in creds_obj, "invalid gs:// creds (missing type)"
...
class AZConfig(StorageConfig):
    ...
    account: str = Field(
        ...
        regex=r"[a-z0-9]+" + re.escape(f".{AZ_BLOB_DOMAIN}"),
    )
    container: str = Field(
