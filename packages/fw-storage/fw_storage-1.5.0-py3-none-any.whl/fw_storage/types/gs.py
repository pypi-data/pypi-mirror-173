...
def create_default_client(service_account_key: str = None) -> Client:
    ...
    with TempEnv() as env:
        key = str(service_account_key or env.get(creds) or "")
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            service_account_key: Google Service Account Key path or contents.
            ...
            write: Pre-check that bucket is writable. Default: False.
        """
        self.bucket = bucket
        self.prefix = prefix.strip("/")
        create_client = create_client or create_default_client
        self.client = create_client(service_account_key)
    ...
    def ls(
        ...
        **_,
        ...
        for blob in self.client.list_blobs(self.bucket, prefix=prefix):
            ...
            if info.path.endswith("/") and info.size == 0:  # skip gs "folders"
