...
if [ "$(echo "intel123" | sudo docker ps -q -f name=^health_influxdb$)" ]; then
    ...
    echo -e "To stop run command: '\e[1;3;4;33mdc edge-deployment-services health-check stop $1\e[0m'"
else
    ...
    if [ $1=="all-services" ]; then      
        docker-compose -f /usr/local/lib/python$ver/dist-packages/src/scripts/edge-deployment-services/health-check/docker-compose.yml  up -d --build influxdb grafana collectd telegraf    
    fi
fi
