...
class Profiling(RapidsTool):
    ...
    def _run_tool_as_spark(self):
        ...
        auto_tuner_args = (
            f"--auto-tuner"
            f" --worker-info {worker_info_path}"
        )
        submit_cmd = (
            ...
            f" --"
            f" {auto_tuner_args}"
            f" --output-directory {self.ctxt.get_remote_output_dir()}"
...
class Qualification(RapidsTool):
    ...
    def __generate_qualification_configs(self) -> str:
        initialization_actions = self.ctxt.get_value('sparkRapids', 'gpu', 'initializationScripts').format(
            self.region, self.region)
        instructions_str = (
            f'To launch a GPU-accelerated cluster with RAPIDS Accelerator for Apache Spark, add the following to your cluster '
            f'creation script:\n\t'
    ...
    def _process_tool_output(self):
        ...
        def report_tool_full_location() -> str:
            out_folder_path = os.path.abspath(self.ctxt.get_local_output_dir())
            qualification_out_msg = (
                f"{self.name.capitalize()} tool output is saved to local disk {out_folder_path}"
                ...
                f"https://nvidia.github.io/spark-rapids/docs/spark-qualification-tool.html"
                f"#understanding-the-qualification-tool-output")
            return qualification_out_msg
        ...
        try:
            ...
            report_summary.print_report(app_name=self.name.capitalize(),
                                        ...
                                        output_pprinter=report_tool_full_location)
        except Exception as ex:
