...
def read_table(spark_session: SparkSession, db_name: str, table_name: str, where=None) -> DataFrame:
    table_name = f"{db_name}.{table_name}"

    if where is None:
        df = spark_session.readStream.format('delta').table(tableName=table_name)
    else:
        df = spark_session.readStream.format('delta').table(tableName=table_name).filter(where)
...
def __write_function(
    ...
    if current_df:
        ...
        if insert_mode == "overwrite":
            truncate_table(spark=spark, db_table=table_name)
        micro_batch_df.write.option("header", "true").option(
            "mergeSchema", "true"
        ).partitionBy(*partition_columns).format("delta").mode(insert_mode).saveAsTable(
            table_name
        )
        spark.sql(f"ALTER TABLE {table_name} OWNER TO `data-platform`")
        print(f"Created table {table_name} and loaded data")
    print(f"Applied insert mode: {insert_mode}")
...
def write_table(
    ...
    if sort_fields and upsert_fields:
        if check_table_exists(spark=spark, db_name=db_name, table_name=table_name):
            ...
            return stream.foreachBatch(upsert_func).start()
        else:
...
def truncate_table(spark, db_table):
    spark.sql(f"TRUNCATE TABLE {db_table}")
    print(f"TABLE {db_table} truncated")
...
def bronze_to_silver(
    ...
    df = read_table(spark, db_name=input_db_name, table_name=input_table_name, where=where)
