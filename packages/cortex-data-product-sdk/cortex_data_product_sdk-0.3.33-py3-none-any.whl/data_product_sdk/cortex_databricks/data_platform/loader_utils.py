...
def __verify_parameters(function):
    def wrapper(*args, **kwargs):
        insert_modes_avalables = ["append", "upsert"]
        if kwargs.get("insert_mode") in insert_modes_avalables:
...
def __write_function(
    ...
    if current_df:
        ...
        micro_batch_df.write.option("header", "true").option(
            "mergeSchema", "true"
        ).partitionBy(*partition_columns).format("delta").mode("append").saveAsTable(
            table_name
        )
...
def landing_to_bronze(
    ...
    :param db_name: database where the data will be written
