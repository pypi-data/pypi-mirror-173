...
def cvt_yolo_annotations_to_coco_json(fdir_label_txts: str, fdir_imgs: str, fpath_dst: str, materials_dict: dict,
    ...
    :return:
    """
    txt_files = ds_annotations.get_all_fpaths_by_extension(fdir_label_txts, (".txt",))
    img_files = ds_annotations.get_all_fpaths_by_extension(fdir_imgs, YOLO_IMG_FORMATS)
...
def get_yolo_img_annotation_pairs(yolo_root_dir: str) -> List[Tuple[str, str]]:
    ...
    pairs = []
    for fpath_img in fpaths_imgs:
        basename_img = ds_annotations.get_basename_only(fpath_img)
        corresponding_txt_files = []
        for fpath_txt in fpaths_txts:
            basename_txt = ds_annotations.get_basename_only(fpath_txt)
