...
@inject_tokens()
async def add_tags_to_records(module: str, data: dict, client: AsyncClient, api: ZohoApi, record_id: str | None = None) -> Response:
...
@inject_tokens()
async def remove_tags_to_records(module: str, data: dict, client: AsyncClient, api: ZohoApi, record_id: str | None = None) -> Response:
    if record_id is None:
...
@inject_tokens()
async def create_tags(params: dict, data: dict, client: AsyncClient, api: ZohoApi) -> Response:
    url = f"{api.domain}/crm/{api.version}/settings/tags"
...
@inject_tokens()
async def update_tags(params: dict, data: dict, client: AsyncClient, api: ZohoApi, tag_id: str | None = None) -> Response:
    if tag_id is None:
        url = f"{api.domain}/crm/{api.version}/settings/tags"
    else:
...
@inject_tokens()
async def get_tag_records_count(tag_id: str, params: dict, client: AsyncClient, api: ZohoApi) -> Response:
...
@inject_tokens()
async def merge_tags(tag_id: str, data: dict, client: AsyncClient, api: ZohoApi) -> Response:
