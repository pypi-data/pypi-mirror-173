...
@inject_tokens()
async def get_variables(client: AsyncClient, api: ZohoApi, variable_id: str | None = None, params: dict | None = None) -> Response:
...
@inject_tokens()
async def get_variable_groups(variable_group: str, client: AsyncClient, api: ZohoApi) -> Response:
...
# variable_group = variable_group_id (or) variable_group_API_name
@inject_tokens()
async def update_variables(data: dict, client: AsyncClient, api: ZohoApi, variable_group: str | None = None) -> Response:
...
@inject_tokens()
async def delete_variables(client: AsyncClient, api: ZohoApi, variable_id: str | None = None, params: dict | None = None) -> Response:
    if variable_id is None:
        url = f"{api.domain}/crm/{api.version}/settings/variables"
    else:
        url = f"{api.domain}/crm/{api.version}/settings/variables/{variable_id}"
