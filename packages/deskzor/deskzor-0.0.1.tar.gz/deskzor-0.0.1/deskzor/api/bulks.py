...
from deskzor.api.utils import get_job_state, get_job_id
from deskzor.core.tokens import inject_tokens, ZohoApi
from deskzor.errors import TimeOut
from deskzor.logs import logs
from deskzor.utils import now_str
from deskzor.env import BULKS_DIR
...
async def get_bulk(filename: str,
                   data: dict,
                   path: str | Path | None = None,
                   filename_only: bool = False) -> Path:
    ...
    final_path = _path / f"{filename}.zip" if filename_only else _path / f"{filename}___{id}_{now_str()}.zip"
