...
class cmr() :
    ...
    def Options(self, sort_key='start_date', scroll=True, page_size=2000,
                ...
                cloud_cover=None, version=None, **kwargs):
        ...
        if bounding_box is not None :
            ...
            new_options += '&bounding_box[]=' + bounding_box
    ...
    def cmr_search(self, starttime, endtime,
                   platform=None, instrument=None,
                   sensor=None, short_name=None, **kwargs):
    ...
    def searchtoken(self, username, password):
        ...
        for item in res :
            ...
            if validdate <= datetime.datetime.utcnow() :
                print('该token有效时间至%s，当前已失效，请删除后重新创建新的token' %(validdate.strftime('%Y-%m-%d')))
        if token is None :
    ...
    def deletetoken(self, token, username, password):
        '''删除token'''
        credentials = '{username}:{password}'.format(username=username,
                                                     password=password)
        credentials = base64.b64encode(credentials.encode('ascii')).decode('ascii')
    ...
    def cmr_download(self, output_dir, url, token=None,
                     username=None, password=None, timeout=5*60, skip=False):
        '''
        根据输入url下载相应的文件
        ...
        filename = self._cmr_download(output_dir, url, token=token,
                                  timeout=timeout, skip=skip)

        return filename

    def _cmr_download(self, output_dir, url, token=None,
                  timeout=5*60, tries=3,
                  chunk_size=1024, skip=False):
        '''通过wget方式进行数据下载'''
        ...
        if platform.system().lower() == 'windows' :
            cmd = f'{WGET} --no-cookie  --tries={tries} --no-check-certificate ' \
