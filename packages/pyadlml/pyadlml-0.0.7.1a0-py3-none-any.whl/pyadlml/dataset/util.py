...
def select_timespan(df_devices=None, df_activities=None, start_time=None, end_time=None, clip_activities=False):
    ...
    df_devices : pd.DataFrame or None
        A device dataframe
    df_activities : pd.DataFrame or None
    ...
    if df_activities is not None:
        if start_time is not None:
            act_start_sel = ~(df_activities[END_TIME] < start_time)
        else:
            act_start_sel = np.full(len(df_activities), True)
        if end_time is not None:
            act_end_sel = ~(df_activities[START_TIME] > end_time)
        else:
            act_end_sel = np.full(len(df_activities), True)

        df_activities = df_activities[act_start_sel & act_end_sel]

        # clip activities if they extend in regions that are not in the timespan
        if not df_activities.empty and clip_activities and start_time is not None \
                and (start_time < df_activities[END_TIME].iat[0] and start_time > df_activities[START_TIME].iat[0]):
            df_activities[START_TIME].iat[0] = start_time
        if not df_activities.empty and clip_activities and end_time is not None \
           and end_time < df_activities[END_TIME].iat[-1]:
           df_activities[END_TIME].iat[-1] = end_time
...
def fetch_data_by_string(dataset: str, cache=True, identifier=None):
    ...
    if dataset == DATASET_STRINGS[0]:
        ...
        data.df_activities = data.df_activities_resident2
