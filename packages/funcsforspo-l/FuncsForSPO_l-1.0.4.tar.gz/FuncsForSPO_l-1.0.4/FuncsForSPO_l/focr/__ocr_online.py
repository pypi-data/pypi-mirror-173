...
from FuncsForSPO_l.fexceptions.exceptions import SessionNotCreatedException
import tempfile
import json
import os
...
class GetTextPDF:    
    def __init__(self, file_pdf: str, dir_exit: str='output', get_text_into_code: bool=True, headless: bool=True, prints: bool=False) -> str:
        ...
        try:
            ...
            self.__service = Service(executable_path=os.path.abspath('FuncsForSPO_l/fselenium/chromedriver'))
            self.DRIVER = Chrome(service=self.__service, options=self._options)
