            ...
            match event.token() {
                token if event.is_writable() => {
                    ...
                    if let Some(mut resp) = self.responses.remove(&token) {
                        ...
                        if Self::handle_write_event(&mut resp, py, thread_state) {
                            // s. https://docs.rs/mio/0.7.11/mio/event/trait.Source.html#dropping-eventsources
                            if let Err(e) = self.poll.registry().deregister(&mut resp.connection) {
...
app = simple_app"#,
        ...
        match app {
            Ok(_) => {
                ...
                non_blocking_worker::<MioTcpListener, WriteMock>(
                    23,
                    shared_wsgi_options(server_name, port, sn.to_string(), false, Some(10), py),
                    app,
                    rcv.clone(),
                    snd.clone(),
                );
    ...
    fn test_handle_events() {
        let mut wstate = WorkerState::<TcpStream>::new(0);
        let token = Token(42);
                ...
                .register(&mut conn, token, Interest::WRITABLE)
            {
                Ok(_) => {
                    ...
                    loop {
                        ...
                        match r.connection.write(&mut buf) {
                            Ok(_) => {
                                wstate.responses.insert(token, r);
                            }
                            Err(e) => {
