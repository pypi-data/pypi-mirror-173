#!/usr/bin/env python
# [[file:../engel_curves.org::+begin_src python :tangle ./consumerdemands/engel_curves.py :shebang "#!/usr/bin/env python"][No heading:1]]
# Tangled on Thu Oct 27 09:29:45 2022
...
def plot(p,alpha,beta,phi,labels=[],ybounds=[0,10],npts=100,fname=None,NegativeDemands=True,use_linestyles=False,shares=False,logs=True,use_figure=1,ax=None):
    ...
    if not shares:
        foo=lambda y: marshallian.demands(y,p,parms,NegativeDemands=NegativeDemands)
    else:
        foo=lambda y: marshallian.budgetshares(y,p,parms,NegativeDemands=NegativeDemands)
...
def plot_demands(p,y,alpha,beta,phi,labels=[],ybounds=[0,10],npts=100,fname=None,NegativeDemands=True,use_linestyles=False,shares=False,logs=True,use_figure=1):
    ...
    if not shares:
        f=lambda p0: marshallian.demands(y,prices(p0),alpha,beta,phi,NegativeDemands=NegativeDemands)
    else:
        f=lambda p0: marshallian.budgetshares(y,prices(p0),alpha,beta,phi,NegativeDemands=NegativeDemands)
