...
class timedelta(base_timedelta):
    ...
    def _parse_duration(cls, duration_string):
        ...
        for char in character_stream:
            ...
            if char == "T":
                if value and designator:
                    msg = f"content after '{designator}' lacks designators"
            ...
            if char not in remaining_designators:
                raise ValueError(f"unexpected character '{char}'")

            designator, measurement, value = char, float(value), ""
            if measurement:
        ...
        if value and designator:
            raise ValueError(f"content after '{designator}' lacks designators")
        if value:
            segment_parser = {
