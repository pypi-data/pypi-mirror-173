...
from exalead_exporter.constants import (PKG_NAME, PKG_VERSION, EXPORTER_CONFIG_NAME)

from exalead_exporter.exalead_api import ExaleadAPI
from exalead_exporter.filters import Filters
from exalead_exporter.core_exporter import ExaleadExporter
from exalead_exporter.collector import ExaleadCollector
from exalead_exporter.yamlscript import YamlScript
...
def main():
   ...
   parser = argparse.ArgumentParser(description='collector for exalead.')
...
#      logger.debug( 'metrics are: {0}'.format(metrics) )
   ...
   #*****************************
   #* build exalead api interface
   if 'exaleads' not in config:
      logger.error('no exalead definitions found in config file.')
      my_exit(1)

   apis = []
   for exalead in config['exaleads']:

      protocol = "http"
      if 'protocol' in exalead:
         protocol = exalead['protocol']

      verify_ssl = True
      if 'verify_ssl' in exalead:
         verify_ssl = exalead['verify_ssl']

      timeout = 10.0
      if 'timeout' in exalead:
         timeout = float(exalead['timeout'])

      keep_session = True
      if 'keep_session' in exalead:
         keep_session = exalead['keep_session']

      port = None
      if 'port' in exalead:
         port = int(exalead['port'])

      labels = None
      if 'default_labels' in exalead:
         labels = exalead['default_labels']

      proxy = None
      if 'proxy' in exalead and isinstance(exalead['proxy'], dict):
         proxy = exalead['proxy']

      #* to do on every "request"
      try:
         api =  ExaleadAPI(
            # ( exalead['user'], exalead['password'] ),
            host=exalead['host'],
            port=port,
            url_path_prefix="mami",
            protocol = protocol,
            verify = verify_ssl,
            timeout = timeout,
         ...
         logger.error('can\'t init exalead_exporter api.')
         my_exit(1);
   ...
   #*****************************
   #* build exalead exporter interface
   exporter = ExaleadExporter(
      engine=script,
      apis=apis,
      metrics=metrics,
      debug=args.verbose,
