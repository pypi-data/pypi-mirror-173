...
async def _get_client(ls: int):
    ...
    client = TonlibClient(ls_index=ls, verbosity_level=1)
    await client.init_tonlib()
    client.tonlib_wrapper.__del__()
    return client
...
def get_items(addresses: list, filename_with_nft_metadata=None, client: TonlibClient=None, max_requests: int = 1000, ls:int=0):
    ...
    for i in range(ceil(len(addresses) / max_requests)):
        ...
        for addr in addresses[i * max_requests:min(len(addresses), max_requests * (i + 1))]:
            tasks.append(get_item(client, addr, nft_items, items, ls))
...
async def get_item(client: TonlibClient, addr: str, nft_items: list, items: dict = None, ls=0):
    if not client.tonlib_wrapper._is_working:
        await client.tonlib_wrapper.close()
        client = await _get_client(ls)
