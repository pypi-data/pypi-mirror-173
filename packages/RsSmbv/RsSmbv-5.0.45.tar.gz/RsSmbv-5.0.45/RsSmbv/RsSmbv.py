...
class RsSmbv:
	"""9288 total commands, 26 Subgroups, 0 group commands"""
	_driver_options = "SupportedInstrModels = SMW/SMBV/SMCV, SupportedIdnPatterns = SMW/SMBV/SMCV, SimulationIdnString = 'Rohde&Schwarz,SMBV100B,100001,5.0.45.0024'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core.driver_version = '5.0.45.0024'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '5.0.45.0024'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsSmbv version failed. Current version: '5.0.45.0024', minimum required version: '{min_version}'")
	...
	@property
	def source(self):
		"""source commands group. 33 Sub-classes, 1 commands."""
	...
	def xfer(self):
		...
		if not hasattr(self, '_xfer'):
			from .Implementations.Xfer import XferCls
			self._xfer = XferCls(self._core, self._cmd_group)
		return self._xfer
