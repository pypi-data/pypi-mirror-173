...
class PacketList(list):
    ...
    def to_csv(self, path, append=False, export_packet_id=True, columns=None, add_sprinkler_info=False):
        ...
        return self.export_packet_df(packet_df=self.get_df(add_sprinkler_info=add_sprinkler_info), path=path, append=append,
        ...
        for index, row in import_packets_df.iterrows():
            try:
                ...
                if 'raw_packet' in row.keys() and 'gw_packet' in row.keys():
                    ...
                    continue
                if reconstructed_packet == '' or reconstructed_packet is None:
                    non_packets_rows.append(index)
                    continue
            except Exception as e:
                non_packets_rows.append(index)
...
if __name__ == '__main__':
    ...
    try:
        ...
        new_list.to_csv(
            path=os.path.abspath('internal/data_samples_for_tests/offline_tester@packets_data_decrypted.csv'))
