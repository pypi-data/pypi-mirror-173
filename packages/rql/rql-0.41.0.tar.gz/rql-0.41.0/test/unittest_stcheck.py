...
class CheckClassTest(unittest.TestCase):
    ...
    def test_rewrite(self):
        for rql, expected in (
            ("Person X", "Any X WHERE X is Person"),
            (
                "Any X WHERE X eid IN (12), X name 'toto'",
                'Any X WHERE X eid 12, X name "toto"',
            ),
            ("Any X WHERE X work_for Y, Y eid 12", "Any X WHERE X work_for 12"),
            ("Any X WHERE Y work_for X, Y eid 12", "Any X WHERE 12 work_for X"),
            ("Any X WHERE X work_for Y, Y eid IN (12)", "Any X WHERE X work_for 12"),
            (
                "Any X ORDERBY Y WHERE X work_for Y, Y eid IN (12)",
                "Any X WHERE X work_for 12",
            ),
            ("Any X WHERE X eid 12", "Any 12"),
            ("Any X WHERE X is Person, X eid 12", "Any 12"),
            (
            ...
            ("Any X WHERE A eid 12, X connait Y", "Any X WHERE X connait Y"),
            (
            ...
            ("Any X WHERE NOT EXISTS(X eid 1)", "Any X WHERE NOT EXISTS(X eid 1)"),
        ):
            with self.subTest(rql=rql):
    ...
    def test_subquery_graphdict(self):
        ...
        rqlst = self.parse(
            (
                "DISTINCT Any VF ORDERBY VCD DESC WHERE "
...
class AnnotateTest(unittest.TestCase):
    ...
    def test_not_rel_normalization_1(self):
        ...
        self.assertEqual(
            rqlst.as_string(), "Any X WHERE C is Company, NOT EXISTS(X work_for C)"
        )
        C = rqlst.defined_vars["C"]
        self.assertFalse(C.scope is rqlst, C.scope)
    ...
    def test_not_rel_normalization_2(self):
        ...
        self.assertEqual(
            rqlst.as_string(), "Any X,ET WHERE C is ET, NOT EXISTS(X work_for C)"
        )
        C = rqlst.defined_vars["C"]
        self.assertTrue(C.scope is rqlst, C.scope)
