...
CSPType = Union_["ConstraintCSPProblem", "GecodeCSPProblem"]
CSPTypeType = Union_[Type["ConstraintCSPProblem"], Type["GecodeCSPProblem"]]

if rql_solve is None:
    CSPProblem: CSPTypeType = ConstraintCSPProblem
else:
...
class ETypeResolver:
    ...
    def __init__(
        ...
        uid_func_mapping: UIDFuncMapping = None,
    ) -> None:
        """
        :Parameters:
        ...
        if uid_func_mapping is None:
            self.uid_func_mapping: UIDFuncMapping = {}
            self.uid_func: Optional[Callable] = None
        else:
    ...
    def set_schema(self, schema: "rql.interfaces.ISchema") -> None:
        ...
        self._nonfinal_domain = set(
            str(etype) for etype in schema.entities() if not etype.final
        )
    ...
    def _visit(
        ...
        constraints: Optional[CSPType] = None,
    ) -> None:
    ...
    def _uid_node_types(self, valnode: "rql.base.LeafNode") -> Set[str]:
        types = set()
    ...
    def _extract_constraint(
        ...
        constraints: CSPType,
        var: str,
        term: "rql.nodes.Constant",
        get_target_types: Callable,
    ) -> None:
        if self.uid_func:
            alltypes = set()
    ...
    def visit_union(self, node: "rql.stmts.Union") -> None:
    ...
    def _visit_insert_or_delete(
        ...
        if not node.defined_vars:
            ...
            return
        constraints = self._init_scope_node(node)
    ...
    def visit_select(self, node: "rql.stmts.Select") -> None:
        ...
        for ca in node.aliases.values():
            etypes = set(
                stmt.selection[ca.colnum].get_type(sol, self.kwargs)
                for stmt in ca.query.children
                for sol in stmt.solutions
            )
        ...
        if isinstance(lhs, nodes.Constant):  # lhs is a constant node (simplified tree)
            ...
            for frometype, toetypes in rschema.associations():
                ...
                if fromtype in lhsdomain:
                    ...
                    res.append([([lhsvar], [str(fromtype)]), ([rhsvar], list(ptypes))])
            ...
            ptypes = [str(subj) for subj in rschema.subjects() if subj in lhsdomain]
            constraints.var_has_types(lhs.name, ptypes)
        return None

    def visit_type_restriction(
        self,
        relation: "rql.nodes.Relation",
        constraints: CSPType,
    ) -> None:
        lhs, rhs = relation.get_parts()
        etypes = set(
            c.value for c in rhs.iget_nodes(nodes.Constant) if c.type == "etype"
        )
        if relation.r_type == "is_instance_of":
            for etype in tuple(etypes):
                for specialization in self.schema.entity_schema_for(
                    etype
                ).specialized_by():
    ...
    def visit_and(self, et: "rql.nodes.And", constraints: CSPType) -> None:
        pass

    def visit_or(self, ou: "rql.nodes.Or", constraints: CSPType) -> None:
        pass

    def visit_not(self, et: "rql.nodes.Not", constraints: CSPType) -> None:
        pass

    def visit_comparison(
        self, comparison: "rql.nodes.Comparison", constraints: CSPType
    ) -> None:
        pass

    def visit_mathexpression(
        self, mathexpression: "rql.nodes.MathExpression", constraints: CSPType
    ) -> None:
        pass

    def visit_function(
        self, function: "rql.nodes.Function", constraints: CSPType
    ) -> None:
        pass

    def visit_variableref(
        self, variableref: "rql.nodes.VariableRef", constraints: CSPType
    ) -> None:
        pass

    def visit_constant(
        self, constant: "rql.nodes.Constant", constraints: CSPType
    ) -> None:
        pass

    def visit_keyword(self, keyword: Any, constraints: CSPType) -> None:
        pass

    def visit_exists(self, exists: "rql.nodes.Exists", constraints: CSPType) -> None:
        pass
...
class ETypeResolverIgnoreTypeRestriction(ETypeResolver):
    ...
    def visit_not(self, et, constraints):
        child = et.children[0]
        if (
            isinstance(child, nodes.Relation)
            and not self.schema.rschema(child.r_type).final
        ):
            return True
