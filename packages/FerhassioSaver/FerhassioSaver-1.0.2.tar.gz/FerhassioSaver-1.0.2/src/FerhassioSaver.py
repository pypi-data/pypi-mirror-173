...
class FerhassioSaver:
    ...
    def __init__(self, hosts, user, password, dbname, coll):
        ...
        assert self.async_db.context == 'async'
        assert self.async_aql.context == 'async'
        assert self.async_coll.context == 'async'
    ...
    def fetch_data(self, cnt=None, field=None) -> list:
        ...
        if cnt:
            job = self.database.aql.execute(f"FOR doc IN {self.async_coll.name} LIMIT {cnt} RETURN doc")
        else:
            job = self.database.aql.execute(f"FOR doc IN {self.async_coll.name} RETURN doc")

        if field:
            fetched_data = [document[field] for document in job]
        else:
    ...
    def load_data(self, json_list: list, chunk_size: int) -> None:
        """
        Save to ArangoDB
        ...
        for chunk in chunk_generator:
            self.async_coll.import_bulk(chunk, on_duplicate='replace')
            self.workers_count += 1

        transaction_end_time = time.time()
        self.time_spent += round(transaction_end_time - transaction_start_time, 5)

    def load_once(self, doc: dict) -> None:
        """
        Load one document
        :param doc: json
        :return: None
        """
        self.async_coll.insert(doc, overwrite_mode='update')
