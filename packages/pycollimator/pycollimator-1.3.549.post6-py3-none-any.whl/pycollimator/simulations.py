...
class SimulationResults:
    def __init__(self, stream, model: Model):
        warnings.warn(
            N(
                """ \x1b[31;20m
This release contains a breaking change in the simulation model representation!
Use a block's full path to access simulation results when using the .to_pandas()
method. If `df = sim.results.to_pandas()`, the column names have changed from
`SubmodelName.BlockName` to `SubmodelName.BlockName.out_0`.
See `sim.results.columns` for a list of all block paths in a model.  \x1b[0m
"""
            )
        )
    ...
    def to_pandas(self, pattern=None, name=None, path=None, type=None, case=False, item=None) -> pd.DataFrame:
        ...
        for col in self._raw_df.columns:
            ...
            block_path = col[: col.rfind(".")]  # remove the port index
            # all outputs of specified blocks included
