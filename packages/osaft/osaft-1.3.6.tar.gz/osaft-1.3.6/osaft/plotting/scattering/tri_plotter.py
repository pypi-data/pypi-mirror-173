...
class TriangulationPlotter(BaseScatteringPlotter):
    ...
    :param particle: if `True`, plot of the particle, else plot of the fluid
    ...
    def plot(
        ...
        Optional[plt.Figure], plt.Axes, TriMesh, Optional[Colorbar],
        tri.Triangulation,
    ]:
        ...
        or `tricontourf
        <https://matplotlib.org/stable/api/_as_gen/matplotlib.pyplot.tricontourf.html>`_
        from Matplotlib.
        ...
        :param tripcolor: if `True` tripcolor plot, if `False` tricontourf plot
    ...
    def animate(
        ...
        **kwargs,
        ...
        fig, ax, triplot, cbar, triang = self.plot(
            Z, X, C_norm, radius, symmetric, tripcolor,
            cbar_label, use_diverging_cmap, ax, vmin=vmin, vmax=vmax, **kwargs,
        )
        ...
        if tripcolor:
            anim = FuncAnimation(
                ...
                blit=False,
                fargs=(
                    frames, triplot, annotation, animate_meth, symmetric,
                    use_diverging_cmap,
                ),
            )
        else:
            anim = FuncAnimation(
                fig, self._animation_func_tricontourf,
                frames=frames,
                interval=interval,
                blit=False,
                fargs=(
                    frames, ax, triplot, triang, annotation, animate_meth,
                    symmetric, use_diverging_cmap,
                ),
            )
    ...
    def triangulation(
        ...
        Y: np.ndarray,
        symmetric: bool,
    ) -> tuple[np.ndarray, np.ndarray, tri.Triangulation]:
    ...
    def masked_triangulation(
        ...
        if not self.particle:
            mask = (
                np.hypot(
                    X[triangs.triangles].mean(axis=1),
                    Y[triangs.triangles].mean(axis=1),
                ) < radius
            )
