...
class BaseScattering(ABC):
    ...
    def radial_acoustic_fluid_velocity(
        self,
        r: Union[float, Sequence],
        theta: Union[float, Sequence],
        t: Union[float, Sequence],
        scattered: bool,
        incident: bool,
    ...
    def velocity_potential(
        ...
        if not scattered and not incident:
            ...
            def inner_func(n: int, r: float):
                x = self.k_f * r
                out = self.field.A_in(n) * Bf.besselj(n, x)
                out += self.potential_coefficient(n) * Bf.hankelh1(n, x)
                return out
        elif scattered and not incident:
            def inner_func(n: int, r: float):
                x = self.k_f * r
                return self.potential_coefficient(n) * Bf.hankelh1(n, x)
        else:
            def inner_func(n: int, r: float):
                x = self.k_f * r
                return self.field.A_in(n) * Bf.besselj(n, x)

        if mode is None:
            out = self.radial_mode_superposition(inner_func, r, theta, t, mode)
        else:
            out = Leg.cos_monomial(mode, theta, inner_func(mode, r))
