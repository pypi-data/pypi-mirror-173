...
class TimeSeriesDataset(DNNTimeSeriesDatasetBase):
    ...
    def _get_or_calculate_statistics(
        ...
        if exp_store is not None:
            if grain == TimeSeriesDataset._NO_GRAIN:
                grain_dict = {TimeSeriesInternal.DUMMY_GRAIN_COLUMN: TimeSeriesInternal.DUMMY_GRAIN_COLUMN}
            else:
                grain_dict = {g_col: X_one_grain.index.get_level_values(g_col)[0] for g_col in grains}
            data_profile = exp_store.metadata.timeseries.get_featurized_data_profile_by_grain(grain_dict)
        if data_profile is None:
            # If we do not have the data profile or we need to apply log transform,
            # calculate statistics here.
            return self._get_statistics(X_one_grain, grain)
        else:
            # Otherwise take statistics from data profile and post process it.
            return self._get_stats_from_profile(X_one_grain, grain, data_profile)
    ...
    def _get_stats_from_profile(
        ...
        columns_to_remove = set(self.numericalized_grain_cols + self.embedded_columns)
        for col_num, col in enumerate(X_one_grain.columns[1:]):
            if col_num in columns_to_remove:
                continue
    ...
    def get_feature_indices_for_normalization(self) -> torch.Tensor:
        """
        Exclude the indices of numericalized grain features and embeddings.

        :return: The indices of features.
        """
        exclude_indices = set(self.embedded_columns + self.numericalized_grain_cols)
