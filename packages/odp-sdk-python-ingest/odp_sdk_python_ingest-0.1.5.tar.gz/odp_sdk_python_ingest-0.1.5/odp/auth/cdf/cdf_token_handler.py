...
class CdfTokenHandler(TokenHandler):
    ...
    def __init__(
        ...
        self.cluster = (
            cdf_cluster
            or os.getenv("FEDERATED_CDF_CLUSTER")
            or self.DEFAULT_CDF_CLUSTER
        )

        login_url = login_url or os.getenv("FEDERATED_CDF_LOGIN_URL")

        if not token_expiry_leeway:
            token_expiry_leeway = timedelta(
                seconds=os.getenv("FEDERATED_CDF_TOKEN_EXPIRY_BUFFER_PERIOD_SECONDS")
                or self.TOKEN_EXPIRY_LEEWAY_PERIOD_DEFAULT_SECONDS
            )
