from typing import Any, Union, Type, Optional, TypeVar, Generic
...
class _ModelRetrieveBase(ModelLogicBaseAPIMixin, Generic[_DjangoModelTypeVar]):
    retrieve_serializer: Type[Serializer]
    instance_pk_field_name: str = "id"

    def serialize_obj(self, obj: _DjangoModelTypeVar) -> dict:
        return self.retrieve_serializer(obj, read_only=True).data

    def get_object(self, request: Request, *args, **kwargs) -> _DjangoModelTypeVar:
...
class ModelListAPIMixin(ApiPaginatedResponseMixin, ModelLogicBaseAPIMixin):
    ...
    def get_list_items_queryset(self, request: Request, *args, **kwargs) -> QuerySet:
        return self.selector_class().list_all()


class ModelRetrieveAPIMixin(
    _ModelRetrieveBase[_DjangoModelTypeVar],
    ModelLogicBaseAPIMixin,
...
class ModelCreateAPIMixin(
    ...
    def perform_create(self, request: Request, *args, request_data: dict, **kwargs) -> _DjangoModelTypeVar:
        service = self.service_class()
        service.create(**request_data)
        return service.instance


class ModelUpdateAPIMixin(
    _ModelRetrieveBase[_DjangoModelTypeVar],
    ModelLogicBaseAPIMixin,
    Generic[_DjangoModelTypeVar],
):
    ...
    def perform_update(self, obj: Model, request: Request, *args, request_data: dict, **kwargs) -> _DjangoModelTypeVar:
        service = self.service_class(instance=obj)
        service.update(**request_data)
        return service.instance


class ModelDestroyAPIMixin(
    _ModelRetrieveBase[_DjangoModelTypeVar],
    ModelLogicBaseAPIMixin,
    Generic[_DjangoModelTypeVar],
):
