...
class CogEnvDecoder:
    def __init__(self, worker_id=1, train=True, allow_multiple_obs=True, env_name="linux_V1/1.x86_64", mat_num=0,
        ...
        self._control_delay = 0.01
        self._lidar_noise = 0.03
        self._lidar_lambda = 6
    ...
    def reset(self, fri_cor=0.1, KP=8, KI=0, KD=2, VK1=0.375, M=3.4, Wheel_I=0.0125, enc_noise=0.01, 
                    control_delay=0.01, lidar_noise=0.03, lidar_lambda=6, obs_mat=False,
                    blue_pose=[-1.0, -1.0, 0.0], red_pose=[-1.0, -1.0, 0.0]):
        ...
        self._engine_Environment_channel.set_float_parameter("Encoder_noise", enc_noise)
        self._engine_Environment_channel.set_float_parameter("blue_x", blue_pose[0])
        self._engine_Environment_channel.set_float_parameter("blue_y", blue_pose[1])
        self._engine_Environment_channel.set_float_parameter("blue_yaw", blue_pose[2])
        self._engine_Environment_channel.set_float_parameter("red_x", red_pose[0])
        self._engine_Environment_channel.set_float_parameter("red_y", red_pose[1])
        self._engine_Environment_channel.set_float_parameter("red_yaw", red_pose[2])

        self._control_delay = control_delay
        self._lidar_noise = lidar_noise
        self._lidar_lambda = lidar_lambda
    ...
    def _GetLaser(self, data):
        ...
        n = len(laser_data)
        noise = self._generate_gaussian_noise(n)
        for i in range(n):
            laser_data[i] += noise[i]
        anomaly_num = np.random.poisson(self._lidar_lambda)
        if n > 2*anomaly_num:
            anomaly_id = np.random.choice(n, anomaly_num)
            for i in range(anomaly_num):
                laser_data[anomaly_id[i]] = 0.0
        return laser_data
    ...
    def _thread_step(self):
        while True:
            ...
            # time_delay = np.random.normal(0, self._control_delay)
            # if time_delay < 0:
            #     time_delay = -time_delay
            # if time_delay > 3*self._control_delay:
            #     time_delay = 3 * self._control_delay
    ...
    def _generate_gaussian_noise(self, n):
        noise = np.random.normal(0, self._lidar_noise, [n, ])
        noise = noise.clip(-self._lidar_noise*3, self._lidar_noise*3)
        return noise
