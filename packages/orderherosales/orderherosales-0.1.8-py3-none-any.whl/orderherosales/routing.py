...
def find_route(raw_data,
    ...
    for g_id in group_ids:
        ...
        route[-1] = len(route) - 1
        arrivals.append(np.array(arrival)[route])
        routes.append(route)
        fares.append(int(tot_fare))

    arrivals_cat = np.concatenate(arrivals)
