...
class BlockHolder:
    ...
    def figure(self, fname, title: str = None, width=None):
        if fname.endswith(".mp4"):
            fig = BlockHolder.new(f'<source src="{fname}" type="video/mp4">')
            fig.wrap('<video width="480" height="320" controls autoplay loop>')
        else:
            extras = f"width={width}" if width is not None else ""
            fig = BlockHolder.new(f'<img src="{fname}" class="img-fluid" {extras}>')
