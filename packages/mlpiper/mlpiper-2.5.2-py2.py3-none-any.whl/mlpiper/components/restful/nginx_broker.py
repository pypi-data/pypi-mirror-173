...
class NginxBroker(Base):
    ...
    def _redhat_platform(self):
        ...
        # support running in a container and its possible the host OS (kernel) could be RedHat
        # but the container is Debian so the most correct way to determine the distro is to
        # inspect the filesystem in some way (either by looking at the package managers or or
        # parsing one of the more modern LSB files).
