...
class DataServiceClient():
    ...
    def load_data_source_local(
        ...
        *,
        ...
        return self._load_data_source_input_file(
            project_id,
            uri,
            name or os.path.splitext(os.path.basename(uri))[0],
            description or uri,
            format,
            creation_reason,
            data_kind=data_kind,
    ...
    def join_rowsets(
        ...
        *,
        ...
        self.logger.info(
            f"Call to join: rowsets {rowsets} on {join_columns} with " +
            join_type_info
        )
        ...
        rowsets_transformed_to_rowset_with_id = [
            messages_pb.RowsetWithId(
                ...
                join_type=(
                    messages_pb.JoinType.DS_JT_INNER
                    if len(join_types) == 0 else join_types[index]
                )
    ...
    def stop_streaming_ingestion(self, streaming_ingestion_id):
