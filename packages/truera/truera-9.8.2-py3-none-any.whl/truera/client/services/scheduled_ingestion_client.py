...
_DEFAULT_SCHEDULE = scheduled_ingestion_pb.Schedule(
    schedule=[
        scheduled_ingestion_pb.CronElement(
            ...
            type=scheduled_ingestion_pb.CronElementType.TDK_MINUTE_OF_HOUR
        )
    ]
)
...
class ScheduledIngestionClient():
    ...
    def _schedule_ingestion(
        ...
        historical_start_timestamp = Timestamp()
        historical_start_timestamp.FromDatetime(
            historical_start_time or datetime.now()
        )

        req = scheduled_ingestion_pb.ScheduleIngestionRequest(
            request_template=request_template,
            schedule=schedule,
            historical_start_time=historical_start_timestamp
        )
    ...
    def build_request_tree(
        ...
        create_split_info = materialize_status_response.original_request.data_info.create_split_info
        if create_split_info and create_split_info.output_split_name:
            create_split_info.output_split_name = override_split_name or create_split_info.output_split_name + "_${uuid()}"
