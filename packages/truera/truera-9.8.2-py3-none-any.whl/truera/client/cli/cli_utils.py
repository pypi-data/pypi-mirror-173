...
def _map_split_type(split_type: str):
    ...
    click.echo(
        ...
        ". Valid types are all, train, test, validate, oot, and custom."
    )
    sys.exit(1)
...
def _get_connection_string(connection_string: str):
...
def _get_auth_details():
    ...
    if env_ctx.username and env_ctx.password:
        return AuthDetails(username=env_ctx.username, password=env_ctx.password)
    if env_ctx.token:
        return AuthDetails(token=env_ctx.token)
...
def format_remaining_list(remaining_list):
    ...
    for remaining in remaining_list:
        ...
        message += f"\tname: {remaining.name}, id:{remaining.id}\n"
