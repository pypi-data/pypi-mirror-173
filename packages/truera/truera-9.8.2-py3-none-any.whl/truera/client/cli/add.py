...
from truera.protobuf.public.metadata_message_types_pb2 import \
    FEATURE_TRANSFORM_TYPE_UNKNOWN
...
def nn_project(
    ...
    UncaughtExceptionHandler.wrap(
        _create_nn_project, project_name, score_type, input_data_format,
        description_json, connection_string, log_level
    )


def _create_nn_project(
    project_name, score_type, input_data_format, description_json,
    connection_string, log_level
):
...
def project(
    ...
    UncaughtExceptionHandler.wrap(
        _create_project, project_name, score_type, description_json,
        connection_string, log_level
    )
...
def data_collection(
    ...
    UncaughtExceptionHandler.wrap(
        _create_data_collection, project_name, data_collection_name,
        connection_string, log_level
    )
...
def split(
    ...
    UncaughtExceptionHandler.wrap(
        _add_split_to_data_collection, project_name, data_collection_name,
        connection_string, split_name, split_type, pre_transform_path,
        post_transform_path, labels_path, extra_data_path, log_level, id_column,
        do_not_use_data_service, split_time_range_begin, split_time_range_end,
        timestamp_col
    )


def _add_split_to_data_collection(
    project_name, data_collection_name, connection_string, split_name,
    split_type, pre_transform_path, post_transform_path, labels_path,
    extra_data_path, log_level, id_column, do_not_use_data_service,
    split_time_range_begin, split_time_range_end, timestamp_col
...
def split_nn(
    ...
    UncaughtExceptionHandler.wrap(
        _add_split_nn_to_data_collection, project_name, data_collection_name,
        connection_string, split_name, split_type, split_dir, log_level
    )


def _add_split_nn_to_data_collection(
    project_name, data_collection_name, connection_string, split_name,
    split_type, split_dir, log_level
):
    logger = cli_logger.create_cli_logger(log_level)
...
def monitoring_super_lite_split(
    ...
    UncaughtExceptionHandler.wrap(
        _add_msl_split_to_data_collection, project_name, data_collection_name,
        connection_string, split_name, split_type, predictions_path,
        labels_path, log_level, id_column, do_not_use_data_service,
        split_time_range_begin, split_time_range_end, timestamp_col
    )


def _add_msl_split_to_data_collection(
    project_name, data_collection_name, connection_string, split_name,
    split_type, predictions_path, labels_path, log_level, id_column,
    do_not_use_data_service, split_time_range_begin, split_time_range_end,
    timestamp_col
    ...
    with cl.ArtifactInteractionClient(ar_client, ds_client, logger) as client:
        ...
        _add_prediction_cache(
            project_name, model_metadata["name"], data_collection_name,
            split_name, predictions_path, project.score_type, False,
            model_metadata["model_output_type"], "cli_client",
            "cli_client_version", connection_string, log_level
        )
...
def model(
    ...
    UncaughtExceptionHandler.wrap(
        _create_model, project_name, model_name, model_type, model_output_type,
        path_to_model, data_collection_name, train_split_name, train_parameters,
        connection_string, log_level
    )


def _create_model(
    project_name: str, model_name: str, model_type: str, model_output_type: str,
    path_to_model: str, data_collection_name: str,
    train_split_name: Optional[str], train_parameters: Optional[Mapping[str,
                                                                        Any]],
    connection_string: str, log_level
):
    logger = cli_logger.create_cli_logger(log_level)
    ar_client = cli_utils.get_artifact_repo_client(connection_string, logger)
    ds_client = cli_utils.get_data_service_client(connection_string, logger)
...
def model_metadata(
    ...
    UncaughtExceptionHandler.wrap(
        _add_model_metadata, project_name, model_name, train_split_name,
        train_parameters, data_collection_name, overwrite, connection_string,
        log_level
    )
...
def feature_list(
    ...
    UncaughtExceptionHandler.wrap(
        _add_feature_list, project_name, data_collection_name,
        feature_description_file, force, connection_string, log_level
    )
...
def split_from_table(
    project_name, data_source_name, table_id, data_collection_name, split_name,
    split_type, approx_row_count, seed, sample_kind, label_col, id_col,
    prediction_col, column_spec_file, split_time_range_begin,
    split_time_range_end, model_name, score_type, connection_string, log_level,
    timestamp_col
):
    UncaughtExceptionHandler.wrap(
        _add_split_from_table, project_name, data_source_name, table_id,
        data_collection_name, split_name, split_type, approx_row_count, seed,
        sample_kind, label_col, id_col, prediction_col, column_spec_file,
        split_time_range_begin, split_time_range_end, model_name, score_type,
        connection_string, log_level, timestamp_col
    )


def _add_split_from_table(
    project_name, data_source_name, table_id, data_collection_name, split_name,
    split_type, approx_row_count, seed, sample_kind, label_col, id_col,
    prediction_col, column_spec_file, split_time_range_begin,
    split_time_range_end, model_name, score_type, connection_string, log_level,
...
def filter_to_table(
    ...
    UncaughtExceptionHandler.wrap(
        _add_filter_to_table, project_name, table_id, data_source_name, filter,
        connection_string, log_level
    )
...
def prediction_cache(
    ...
    UncaughtExceptionHandler.wrap(
        _add_prediction_cache, project_name, model_name, data_collection_name,
        split_name, path_to_prediction_cache, score_type, create_model,
        model_output_type, client_name, client_version, connection_string,
        log_level
    )


def _add_prediction_cache(
    project_name, model_name, data_collection_name, split_name,
    path_to_prediction_cache, score_type, create_model, model_output_type,
    client_name, client_version, connection_string, log_level
):
...
def explanation_cache(
    ...
    UncaughtExceptionHandler.wrap(
        _add_explanation_cache, project_name, model_name, data_collection_name,
        split_name, path_to_explanation_cache, score_type, create_model,
        model_output_type, client_name, client_version, connection_string,
        log_level
    )


def _add_explanation_cache(
    project_name, model_name, data_collection_name, split_name,
    path_to_explanation_cache, score_type, create_model, model_output_type,
    client_name, client_version, connection_string, log_level
):
...
def manual_backup(connection_string, log_level):
    UncaughtExceptionHandler.wrap(_trigger_backup, connection_string, log_level)


def _trigger_backup(connection_string, log_level):
    logger = cli_logger.create_cli_logger(log_level)
...
@click.option("--log_level", default=None, help=cli_logger.log_level_message)
def labels(
    project_name, data_collection_name, table_id, data_source_name, split_name,
    approx_row_count, seed, sample_kind, label_column, id_column,
    connection_string, log_level
):
    UncaughtExceptionHandler.wrap(
        _ingest_delayed, project_name, data_collection_name, table_id,
        data_source_name, split_name, approx_row_count, seed, sample_kind,
        label_column, None, id_column, connection_string, log_level
    )
...
def extra_data(
    project_name, data_collection_name, table_id, data_source_name, split_name,
    approx_row_count, seed, sample_kind, extra_data_column, id_column,
    connection_string, log_level
):
    UncaughtExceptionHandler.wrap(
        _ingest_delayed, project_name, data_collection_name, table_id,
        data_source_name, split_name, approx_row_count, seed, sample_kind, None,
        [extra_data_column], id_column, connection_string, log_level
    )


def _ingest_delayed(
    project_name, data_collection_name, table_id, data_source_name, split_name,
    approx_row_count, seed, sample_kind, label_column, extra_columns, id_column,
    connection_string, log_level
):
    logger = cli_logger.create_cli_logger(log_level)
    ar_client = cli_utils.get_artifact_repo_client(connection_string, logger)
