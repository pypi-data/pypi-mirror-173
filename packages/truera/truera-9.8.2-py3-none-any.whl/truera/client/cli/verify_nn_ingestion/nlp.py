...
class NLPVerifyHelper(VerifyHelper):
    ...
    def verify_evaluate_model(
        ...
        if not probits.shape[class_dimension_idx
...
class NLPAutowrapVerifyHelper(NLPVerifyHelper):
    ...
    def verify_autowrap_databatch_to_trubatch(
        self,
        ...
        """
        Autowrapper verification for databatch_to_trubatch method return value.
        """
        trubatch = databatch_to_trubatch(databatch)
    ...
    def verify_autowrap_text_to_inputs(
        self, text_to_inputs: Callable, trubatch: dict
    ):
        """
        ...
        try:
            self.model(*inputbatch['args'], **inputbatch['kwargs'])
        except:
            print(
                "ERROR: Outputs of text_to_inputs does not pass nicely into model"
            )
            return
    ...
    def verify_autowrap_text_to_spans(
        ...
        assert np.all(spanss[:, :, 0] <= spanss[:, :, 1])
    ...
    def verify_flow(
        ...
        trubatch = self.verify_autowrap_databatch_to_trubatch(
            databatch_to_trubatch, databatch, expected_batch_size=batch_size
        )
