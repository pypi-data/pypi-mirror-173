...
class Types(base.Types):
    ...
    class TruBatch(base.Types.TruBatch):
        """
        Instance data required for Truera NLP product.
        """
...
class Wrappers(base.Wrappers):
    ...
    # Include alias here so help(NLP) mentions it.
    Types = Types
    ...
    class WithModelRunWrapperUtility:
        ...
        @Wrapping.utility
        def inputbatch_of_textbatch(
            cls, texts: Types.TextBatch, *, model: NNB.Model,
        ...
        def textbatch_of_databatch(
            cls, databatch: Types.DataBatch, *, model: NNB.Model,
            tokenizer: Wrappers.TokenizerWrapper
        ) -> Types.TextBatch:
            """
            ...
            trubatch = cls.trubatch_of_databatch( # pylint: disable=E1101
                databatch, model=model, tokenizer=tokenizer
            )
            return cls.textbatch_of_trubatch( # pylint: disable=E1101
                trubatch
            )
        ...
        def inputbatch_of_databatch(
            ...
            trubatch = cls.trubatch_of_databatch(databatch, model=model)  # pylint: disable=E1101
            textbatch = cls.textbatch_of_trubatch(trubatch)
        ...
        def evaluate_model_from_text(
            cls, model: NNB.Model, text: Types.Text,
            tokenizer: Wrappers.TokenizerWrapper
        ) -> Types.OutputBatch:
            """
        ...
        def evaluate_model_from_texts(
            cls, model: NNB.Model, texts: Iterable[Types.Text],
            tokenizer: Wrappers.TokenizerWrapper
        ) -> Types.OutputBatch:
            """
            ...
            inputs: Types.InputBatch = cls.inputbatch_of_textbatch(
                texts=texts, model=model, tokenizer=tokenizer
            )

            return cls.evaluate_model(model=model, inputs=inputs)  # pylint: disable=E1101
