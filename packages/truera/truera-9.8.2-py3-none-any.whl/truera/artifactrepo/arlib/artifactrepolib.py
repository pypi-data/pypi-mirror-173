...
class InvalidMapOptionError(Exception):
...
def map_model_output_type(model_output_type):
    ...
    if type(model_output_type) == type(
        md_pb.ModelOutputType.UNKNOWN_MODELOUTPUTTYPE
    ):
        return model_output_type

    if model_output_type.lower() == "classification":
        return md_pb.ModelOutputType.CLASSIFICATION
    if model_output_type.lower() == "regression":
        return md_pb.ModelOutputType.REGRESSION
    if model_output_type.lower() == "default":
        return md_pb.ModelOutputType.UNKNOWN_MODELOUTPUTTYPE
    else:
