...
class PaymentWechatService:
    ...
    def payment_applets_pay(params):
        ...
        try:
            ...
            order = pay.order.create(
                ...
                total_fee=int(total_fee),  # 标价金额，订单总金额，单位为分
    ...
    def payment_refund(params):
        ...
        order = pay.refund.apply(
            ...
            refund_fee=params['refund_fee'],  # 退款总金额，单位为分
            out_refund_no=params['out_trade_no'],  # 商户系统内部的退款单号，商户系统内部唯一，同一退款单号多次请求只退一笔
            out_trade_no="202210261124372361",  # 可选，微信订单号
        )
        return order
    ...
    def payment_logic_processing(param):
        ...
        try:
            ...
            FinanceTransactService.post(finance_data_sand_box)
            if err_txt:
                # print(err_txt)
            ...
            if finance_data:
                ...
                FinanceTransactService.post(platform_revenue_data_sand_box)
                # 更改支付记录
