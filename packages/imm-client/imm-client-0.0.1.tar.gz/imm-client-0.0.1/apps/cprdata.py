...
import os, sys
from apps.config import print_errors, session
...
def getAppActions(pa):
    ...
    r = session.post(SERVER_URL + "pr/pickapp", json=request)
...
def getFormActions(pa, sp, dps, model):
    ...
    r = session.post(SERVER_URL + "pr/webform", json=request)
    if r.status_code == 200:
        return r.json()
    else:
        print_errors(r)
...
def login_prportal(rcic: str):
    ...
    r = session.post(SERVER_URL + "pr/login", json=rcic_account)
    if r.status_code == 200:
        return r.json()
    else:
        print_errors(r)
...
def makeDocx(persons, docx_file):
    ...
    r = session.post(SERVER_URL + "pr/appendix", json=request)
