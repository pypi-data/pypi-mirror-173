...
from system.config import Default, SERVER_URL
...
def make(model: str, output_excel_name: str, language: str = "English"):
    ...
    r = session.post(SERVER_URL + "case/make", json=request, headers=headers)
...
def check(
    ...
    request = {"model": model, "data": getJsonData(excel_name), "language": language}
    r = session.post(SERVER_URL + "case/check", json=request, headers=headers)
...
def run(model: str, excel_name: str, language: str = "English"):
    ...
    excel_name = append_ext(excel_name, ".xlsx")
    request = {"model": model, "data": getJsonData(excel_name), "language": language}
    r = session.post(SERVER_URL + "case/run", json=request)
...
def word(
    ...
    r = session.post(SERVER_URL + "case/word", json=request)
...
def webform(
    ...
    r = session.post(SERVER_URL + "case/webform", json=request)
...
def pdfform(
    ...
    r = session.post(SERVER_URL + "case/pdfform", json=request)
...
def ec(
    ...
    r1 = session.post(SERVER_URL + "case/get_companies", json=request1)
    if r1.status_code == 200:
        ...
        r2 = session.post(SERVER_URL + "case/get_certificate", json=request2)
