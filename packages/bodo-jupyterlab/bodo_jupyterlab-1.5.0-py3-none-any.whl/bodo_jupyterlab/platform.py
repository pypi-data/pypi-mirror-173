...
class PlatformClusterManager:
    ...
    cluster_list_cache = TTLCache(maxsize=2, ttl=GET_CLUSTER_LIST_REFRESH_PERIOD_SECONDS)
    ...
    def _fetch_clusters_list(cls, logger):
        ...
        try:
            ...
            for attempt in range(GET_CLUSTER_INFO_MAX_RETRIES):
                logger.info(f"{logging_prefix} Getting clusters list {attempt + 1} attempt")
                ...
                if PlatformTokensManager.is_access_token_expired(response):
                    logger.info(f"{logging_prefix} Token expired, refreshing access token...")
                    access_token = PlatformTokensManager.refresh_access_token()

            if response is None or not response.ok:
                raise Exception(f"Could not get cluster list from platform! Response: {response}")
            ...
            logger.info(f"{logging_prefix} Cluster list received from backend: {clusters}")
            ...
            logger.error(f"{logging_prefix} Error in PlatformClusterManager.get_clusters_list: {e}")
            raise
    ...
    def resume_cluster(cls, cluster_uuid: str, logger):
        ...
        try:
            ...
            for attempt in range(RESUME_CLUSTER_MAX_RETRIES):
                logger.info(f"{logging_prefix} Resuming cluster: {cluster_uuid} {attempt + 1} attempt")
            ...
            if response is None or not response.ok:
                raise Exception(f"Could not get resume cluster: {cluster_uuid}! Response: {response}")

        except Exception as e:
            logger.error(f"{logging_prefix} Error in PlatformClusterManager.resume_cluster: {e}")
            raise
    ...
    def pause_cluster(cls, cluster_uuid: str, logger):
        ...
        try:
            ...
            for attempt in range(PAUSE_CLUSTER_MAX_RETRIES):
                logger.info(f"{logging_prefix} Pausing cluster: {cluster_uuid} {attempt + 1} attempt")
            ...
            if response is None or not response.ok:
                raise Exception(f"Could not get pause cluster: {cluster_uuid}! Response: {response}")

        except Exception as e:
            logger.error(f"{logging_prefix} Error in PlatformClusterManager.pause_cluster: {e}")
            raise
    ...
    def get_cluster_info(cls, cluster_uuid: str, logger):
        ...
        try:
            ...
            for attempt in range(GET_CLUSTER_INFO_MAX_RETRIES):
                logger.info(f"{logging_prefix} Getting cluster: {cluster_uuid} info {attempt + 1} attempt")
            ...
            if response is None or not response.ok:
                raise Exception(f"Could not get cluster: {cluster_uuid} info! Response: {response}")

            cluster_info = json.loads(response.content.decode("utf-8"))
            logger.info(f"{logging_prefix} Cluster Info received from backend: {cluster_info}")
            return cluster_info

        except Exception as e:
            logger.error(f"{logging_prefix} Error in PlatformClusterManager.get_cluster_info: {e}")
            raise
