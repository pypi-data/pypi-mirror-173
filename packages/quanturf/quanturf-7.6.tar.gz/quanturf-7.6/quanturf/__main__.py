...
if len(site_package_dir) == 1:
    ...
    logo_file_path=os.path.join(site_package_dir, "quanturf", "quanturf_logo.py")
    logo_file_path = logo_file_path.replace(os.sep, '/')
    logo_file=open(logo_file_path,"r")
    logo_code=logo_file.read()
    logo_file.close()
    os.chdir("../")
    os.chdir("../")
    env_dir=os.getcwd()
...
if len(site_package_dir) >= 2:
    os.chdir(site_package_dir[1])
    logo_file_path=os.path.join(site_package_dir[1], "quanturf", "quanturf_logo.py")
    logo_file_path = logo_file_path.replace(os.sep, '/')
    logo_file=open(logo_file_path,"r")
    logo_code=logo_file.read()
    logo_file.close()
