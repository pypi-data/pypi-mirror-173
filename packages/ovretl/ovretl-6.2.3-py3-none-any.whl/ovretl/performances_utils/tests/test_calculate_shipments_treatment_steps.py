...
from ovretl.performances_utils.calculate_shipments_treatment_steps import calculate_shipments_treatment_steps

cwd = os.path.dirname(__file__)


def test_calculate_shipments_treatment_steps():
    ...
    result = calculate_shipments_treatment_steps(
    ...
    result_should_be = pd.read_csv(
        os.path.join(cwd, "shipments_treatment_steps.csv"), parse_dates=[i for i in range(1, 11)]
    )
