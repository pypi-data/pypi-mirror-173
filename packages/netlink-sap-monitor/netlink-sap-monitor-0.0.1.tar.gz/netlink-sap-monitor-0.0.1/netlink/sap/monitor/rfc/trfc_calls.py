...
def transactional_rfc_calls(rfc_connection):
    ...
    for row in rfc_connection.select('ARFCSSTATE',
                                     'ARFCDEST', 'ARFCSTATE', 'ARFCFNAM', 'ARFCUZEIT', 'ARFCDATUM', 'ARFCUSER', 'ARFCTCODE'):
        system_datetime = datetime.datetime.combine(datetime.datetime.strptime(row.arfcdatum, '%Y%m%d').date(),
    ...
    data = defaultdict(                                      # age bracket
        lambda: defaultdict(                                 # destination
            lambda: defaultdict(                             # state
                lambda: defaultdict(                         # function module
                    lambda: defaultdict(                     # user
                        lambda: defaultdict(lambda: 0))))))  # t-code
