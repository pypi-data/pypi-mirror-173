...
def prepare_run_test_offline(username="", password=None, environment="production", test_instance_id="", test_id="", test_alias="", test_version="", client_obj=None, base_folder=None, overwrite=False, **params):
    ...
    base_folder : string
        Base directory for saving the test config and observation file. If not specified,
        will create a directory involving the test UUID and timestamp.
    overwrite : boolean
        Specify if any files at target location should be overwritten or not. Default is False.
    **params : list
    ...
    for test_observation_path in test_observation_paths:
        ...
        test_observation_file = datastore.download_data([test_observation_path], local_directory=base_folder, overwrite=overwrite)[0]
    ...
    if os.path.exists(test_config_file) and not overwrite:
        raise FileExistsError("Target file path `{}` already exists!\nSet `overwrite=True` if you wish to overwrite existing files!".format(test_config_file))
