...
def short(url: str):
    if not exists(EB_CONFIG_FILE_PATH) or isdir(EB_CONFIG_FILE_PATH):
        typer.secho("~./eb-config.json file is missing, use add-key command to generate it", fg=typer.colors.RED)
    else:
        with open(EB_CONFIG_FILE_PATH, "r") as eb_config_file:
            eb_config_dict = json.load(eb_config_file)
        
        if not CONFIG_FILE_API_KEY_KEY in eb_config_dict:
            typer.secho(f"~./eb-config.json does not contain {CONFIG_FILE_API_KEY_KEY} entry, use add-key command to generate it", fg=typer.colors.RED)
        else:
            api_key = eb_config_dict[CONFIG_FILE_API_KEY_KEY]
            payload = {"longUrl": url}

            response = requests.post(url=SHORTENER_URL, json=payload, headers={"Content-Type":"application/json","X-Api-Key":api_key})

            if response.status_code == 200:
                typer.secho(response.json()["shortUrl"], fg=typer.colors.GREEN)

            else:
                typer.secho("something went wront, do you have a valid api key", fg=typer.colors.RED)
