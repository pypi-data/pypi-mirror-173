...
from azureml.automl.dnn.vision.classification.common.constants import (
    ModelNames, ModelParameters)
from azureml.automl.dnn.vision.common.logging_utils import get_logger
from azureml.automl.dnn.vision.explainability.constants import (
    ExplainabilityDefaults, ExplainabilityLiterals,
    IntegratedGradientsDefaults, NoiseTunnelDefaults, XAIPredictionLiterals)
from azureml.automl.dnn.vision.explainability.utils import (
    XaiMultiLabelCustomDataset, save_captum_visualizations)
from azureml.automl.dnn.vision.explainability.xrai_utils import \
    xrai_utility_method
from captum.attr import (GuidedBackprop, GuidedGradCam, IntegratedGradients,
                         NoiseTunnel)
...
def get_explanations_guidedbackprop(gbp_model, input_final, pred_label_idx):
    ...
    :rtype: torch.Tensor
    """
    logger.info("Generating attributions using: guidedbackprop")
...
def get_explanations_guidedgradcam(ggc_model, input_final, pred_label_idx):
    ...
    :rtype: torch.Tensor
    """
    logger.info("Generating attributions using: guidedgradcam")
...
def load_xai_method(model_wrapper, xai_method_name):
    ...
    if xai_method_name == ExplainabilityLiterals.INTEGRATEDGRADIENTS_METHOD_NAME:
        ...
        raise ValueError("invalid XAI method")
...
def get_explanations(
    ...
    **kwargs
    ...
    if xai_method_name == ExplainabilityLiterals.GUIDEDGRADCAM_METHOD_NAME:
        ...
        raise ValueError("invalid xai method name")
    ...
    if kwargs.get(
        ...
        return visualization_image_strs, [
            attr_sample.cpu().detach().numpy().tolist() for attr_sample in attributions
        ]
