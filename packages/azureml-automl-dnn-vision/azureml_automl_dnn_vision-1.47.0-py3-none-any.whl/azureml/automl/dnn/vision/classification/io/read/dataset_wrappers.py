...
class AmlDatasetWrapper(_CommonImageDatasetWrapper, AmlDatasetBaseWrapper):
    ...
    def _validate_dataset(self, images_df, multilabel, ignore_data_errors):
        ...
        def is_valid_label(label):
            if not multilabel:
                label = [label]
            return isinstance(label, list) and all(isinstance(c, str) or isinstance(c, int) for c in label)
        indices_to_drop = [
        ...
        if indices_to_drop:
            if multilabel:
                base_mesg = "Label should be a list of strings or integers for multilabel. " \
                            "Found some datapoints with single label or label other type than str/int. "
            else:
