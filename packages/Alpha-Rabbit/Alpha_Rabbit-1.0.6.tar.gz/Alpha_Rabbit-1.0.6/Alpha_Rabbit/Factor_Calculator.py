from Alpha_Rabbit import Factor_Def_and_Get_Method as FDGM
_method = FDGM.Factor_get_method()
def Factor_Calculator(pricebyday,minbar,conn,todaydate,notst,factors_to_cal):
    ######################################## 日间数据计算因子 ####################################
    uploadfactordict = {}
    Close = pricebyday['close'].unstack()
    Open = pricebyday['open'].unstack()
    High = pricebyday['high'].unstack()
    Low = pricebyday['low'].unstack()
    volume = pricebyday[['volume']].pivot_table(index = 'date',columns = 'symbol',values = 'volume')
    total_turnover = pricebyday[['total_turnover']].pivot_table(index = 'date',columns = 'symbol',values = 'total_turnover')
