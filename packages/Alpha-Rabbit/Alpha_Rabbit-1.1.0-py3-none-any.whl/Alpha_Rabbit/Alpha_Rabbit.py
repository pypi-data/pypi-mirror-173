...
class Barra_factor_ana(object):
    ...
    def __init__(self,df=None,start_date=None,end_date=None,dir=None,skip_fileload=None) -> None:
        ...
        if not skip_fileload:
            ...
            for f in filelist:
                if f[-3:]=='csv':
                    if f[:-4] == 'market_r':
                        self.filedict[f[:-4]] = pd.read_csv(dir+f,index_col = 0)
                    else:
    ...
    def rise_barra_factors(self,rank_normalize:bool):
        ...
        RVN = RVN.stack()
        self.growth = pd.DataFrame(NP['net_profit_parent_company_ttm_0']*0.34+RVN['net_profit_parent_company_ttm_0']*0.66)
