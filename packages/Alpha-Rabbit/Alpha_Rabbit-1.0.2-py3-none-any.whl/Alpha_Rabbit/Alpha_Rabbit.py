...
class single_signal_test(object):
    ...
    def one_factor_return(self,df,factorname,ndays,return_col): # 计算分组收益
        ...
        qmean = qreturn.mean().droplevel(0)
        qcum = qreturn.cumsum()
        qcum.columns = [i[1] for i in list(qcum)]
        return qmean,qcum
...
class multi_factor_test(object):
    ...
    def del_updown_limit(self,factordf,daybar):
        # 剔除涨跌停
        notuplimit = daybar[~(daybar.close == daybar.limit_up)].swaplevel(1,0)
        notdownlimit = daybar[~(daybar.close == daybar.limit_down)].swaplevel(1,0)
...
class Barra_factor_ana(object):
    ...
    def rise_barra_factors(self,rank_normalize:bool):
        ...
        # Bp
        print('rise Bp')
        self.Bp = self.filedict['Bp'].swaplevel(0,1).dropna()
        ...
        self.earning_yield = self.earning_yield[['earning_yield']].dropna().swaplevel(0,1)
        
        # growth
        print('rise growth')
        NP = self.filedict['NPGO'].unstack().T
        NP = (NP-NP.shift(504))/NP.shift(504).abs()
        NP = NP.droplevel(0).stack()
        RVN = self.filedict['RGO'].unstack().T
        RVN = (RVN - RVN.shift(504))/RVN.shift(504).abs()
        RVN = RVN.droplevel(0).stack()
        self.growth = pd.DataFrame(NP*0.34+RVN*0.66)
        ...
        self.leverage = self.leverage[['leverage']].dropna().swaplevel(0,1)
