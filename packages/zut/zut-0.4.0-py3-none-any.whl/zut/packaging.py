...
class BuildMixin:
    ...
    def upload(self, repository=None):
        ...
        logger.error(f"uploading to repository \"{repository}\": {', '.join(builds)}")
        cp = subprocess.run(command, text=True, stdout=sys.stdout, stderr=sys.stderr)
        if cp.returncode != 0:
    ...
    def build(self, upload = False, repository = None):
        ...
        subprocess.run([sys.executable, "-m", "pip", "wheel", "-w", "dist", "."], check=True, stdout=sys.stdout, stderr=sys.stderr)
