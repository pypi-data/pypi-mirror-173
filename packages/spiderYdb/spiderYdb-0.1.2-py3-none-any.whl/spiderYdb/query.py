    ...
    def save_item(cls, item):
        ...
        for field in item.attrs:
            if field.need_update:
                ...
                params[f'${field.name}'] = field.value
                declare.append(f"DECLARE ${field.name} AS {field.title};")
        print(declare, params, added)
        ...
        for field in item.fields:
            if field[0] in item.attrs_dict:
                ...
                declare.append(f"DECLARE ${field[0]} AS {item.attrs_dict[field[0]].title};")
        for _ in item.attrs:
            if _.pk:
                if _.name not in added:
                    if isinstance(_, Uuid):
                        ...
                        declare.append(f"DECLARE ${_.name} AS {_.title};")
