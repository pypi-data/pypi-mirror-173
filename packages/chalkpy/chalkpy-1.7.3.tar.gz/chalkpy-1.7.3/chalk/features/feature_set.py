...
def _init_param(f: Feature):
    ...
    # variable set to 3).
    return f"{f.attribute_name}:_type_{f.name}=MISSING"
...
def _repr_fn(fields: List[Feature], _globals: Mapping[str, Any]):
    tuples = ",".join(f"('{f.attribute_name}', self.{f.attribute_name})" for f in fields)
    fn = create_fn(
        name="__repr__",
        args=["self"],
        body=[
...
def _tuple_str(obj_name: str, fields: Sequence[Feature]):
    ...
    return f'({",".join([f"{obj_name}.{f.attribute_name}" for f in fields])},)'
...
def _len_fn(_globals: Mapping[str, Any]):
    return create_fn(
        ...
        body=[
            ...
            "    if getattr(self, f.attribute_name) != MISSING:",
            "        count += 1",
            "return count",
        ],
        _globals=_globals,
