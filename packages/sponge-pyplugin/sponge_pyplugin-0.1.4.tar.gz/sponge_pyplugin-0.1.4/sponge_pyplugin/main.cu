...
void CV_PYTHON::Initial(COLLECTIVE_VARIABLE_CONTROLLER* manager, int atom_numbers, const char* module_name)
{
    ...
    sprintf(buffer, "Sponge.COLLECTIVE_VARIABLE_PROTOTYPE.New('%s')", module_name); 
...
{
    char buffer[CHAR_LENGTH_MAX + 60];
    sprintf(buffer, "Sponge.COLLECTIVE_VARIABLE_PROTOTYPE.objects['%s'].Compute(%d)", module_name, need); 
    PyRun_SimpleString(buffer);
}
...
class COLLECTIVE_VARIABLE_PROTOTYPE:
    ...
    @classmethod
    def New(cls, module_name):
        type_name = Sponge.cv_controller.Command(module_name, "type")
        cls.objects[module_name] = cls.types[type_name](module_name)
