...
bool Cuda_Malloc_Safely(void **address, size_t size)
{
    ...
    {
        printf("\nError occured in cudaMalloc:\n   cudaError_t %d - %s: %s\n", cuda_error, cudaGetErrorName(cuda_error), cudaGetErrorString(cuda_error));
        getchar();
        return false;
    }
}
...
{   
    ...
    if (cuda_error != 0)
    {
        printf("\nError occured in [%s] cudaMalloc:\n   cudaError_t %d - %s: %s\n", var_name, cuda_error, cudaGetErrorName(cuda_error), cudaGetErrorString(cuda_error));
        getchar();
        return false;
    }
