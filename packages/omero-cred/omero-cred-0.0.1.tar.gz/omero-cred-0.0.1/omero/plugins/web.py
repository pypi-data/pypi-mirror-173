...
def config_required(func):
    """Decorator validating Django dependencies and /settings.py"""
...
class WebControl(DiagnosticsControl):
    ...
    def config(self, args, settings):
        ...
        d = {
            "ROOT": self.ctx.dir,
            "OMEROWEBROOT": self._get_python_dir() / "",
        ...
        c = bytes_to_native_str(
            resource_string("", "templates/" + template_file)
        )
        self.ctx.out(c % d)
    ...
    def enableapp(self, args, settings):
        ...
        if not args.appname:
            ...
            iapps = map(
                lambda x: x.startswith(".") and x[9:] or x,
