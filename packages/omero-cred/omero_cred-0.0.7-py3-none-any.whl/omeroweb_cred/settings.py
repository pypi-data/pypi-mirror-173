...
from omeroweb_cred.utils import sort_properties_to_tuple
from omeroweb_cred.connector import Server
...
# logging.CRITICAL
# FORMAT: 2010-01-01 00:00:00,000 INFO  [.webadmin.webadmin_utils]
...
DEFAULT_SESSION_ENGINE = ".filesessionstore"
...
ROOT_URLCONF = "omeroweb_cred.urls"
...
TEMPLATES = [
    {
        ...
        "APP_DIRS": True,
        "OPTIONS": {
            "builtins": ["omeroweb_cred.webgateway.templatetags.defaulttags"],
            "debug": DEBUG,  # noqa
            "context_processors": [
                ...
                "omeroweb_cred.custom_context_processor.url_suffix",
                "omeroweb_cred.custom_context_processor.base_include_template",
            ],
        },
    },
]
...
for app in ADDITIONAL_APPS:  # from CUSTOM_SETTINGS_MAPPINGS  # noqa
    # Previously the app was added to INSTALLED_APPS as '.app', which
    # then required the app to reside within or be symlinked from within
    # , instead of just having to be somewhere on the python path.
    # To allow apps to just be on the path, but keep it backwards compatible,
    # try to import as .app, if it works, keep that in INSTALLED_APPS,
    # otherwise add it to INSTALLED_APPS just with its own name.

    try:
        __import__("omeroweb_cred.%s" % app)
        INSTALLED_APPS += ("omeroweb_cred.%s" % app,)
    except ImportError:
        INSTALLED_APPS += (app,)
    try:
...
INSTALLED_APPS += (
    "omeroweb_cred.feedback",
    "omeroweb_cred.webadmin",
    "omeroweb_cred.webclient",
    "omeroweb_cred.webgateway",
    "omeroweb_cred.webredirect",
    "omeroweb_cred.api",
    "pipeline",
)
...
PIPELINE = {
    "STYLESHEETS": {
        "webgateway_viewer": {
            ...
            "output_filename": ".viewer.min.css",
        },
    },
...
CSRF_FAILURE_VIEW = ".feedback.views.csrf_failure"
...
INSTALLED_APPS=[('django.contrib.staticfiles', 'django.contrib.auth', 'django.contrib.contenttypes', 'django.contrib.sessions', 'django.contrib.sites', 'omeroweb_cred.feedback', 'omeroweb_cred.webadmin', 'omeroweb_cred.webclient', 'omeroweb_cred.webgateway', 'omeroweb_cred.webredirect', 'omeroweb_cred.api', 'pipeline')]
