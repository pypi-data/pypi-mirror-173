...
def instance_extractor(df: pd.DataFrame, 
...
def to_dicts(triples: Iterator[Tuple[KT, IT, FrozenSet[LT]]]) -> Tuple[Mapping[KT, IT], Mapping[KT, FrozenSet[LT]]]:
...
def one_hot_encoded_extractor(*cols: str) -> Callable[[pd.Series], FrozenSet[str]]:
...
def no_vector() -> Callable[[pd.Series], Optional[np.ndarray]]:
    def callable(row: pd.Series) -> Optional[np.ndarray]:
        return None
    return callable
...
def text_builder(identifier: KT,
                 data: str, 
                 vector: VT, 
                 representation: str, 
                 row: pd.Series,
                 idx: Any) -> MemoryTextInstance[KT, VT]:
...
def text_from_pandas_multilabel(
    ...
    def instances() -> Iterator[Tuple[str, MemoryTextInstance[str, Any], FrozenSet[str]]]:
