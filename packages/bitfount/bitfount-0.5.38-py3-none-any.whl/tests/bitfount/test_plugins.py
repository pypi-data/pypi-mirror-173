...
BITFOUNT_FEDERATED_ALG_MODULE_PATH = f"{BITFOUNT_FEDERATED_MODULE_PATH}.algorithms"
...
class TestAlgorithmPlugins:
    ...
    def dummy_plugin(self, dummy_alg_plugin_name: str) -> str:
        ...
        from bitfount.federated.algorithms.model_algorithms.base import _BaseWorkerModelAlgorithm, \
         _BaseModelAlgorithmFactory, _BaseModellerModelAlgorithm
    ...
    def dummy_alg_plugin(self, dummy_plugin: str, dummy_alg_plugin_name: str) -> str:
        """Returns a class that subclasses BaseAlgorithmFactory."""
        return dummy_plugin.replace(
    ...
    def test_plugin_in_wrong_dir_gets_ignored(
        ...
        # `data_sources` is intentionally the wrong directory, should be `algorithms`
        dummy_plugin_path = bitfount.config.BITFOUNT_FEDERATED_PLUGIN_PATH / "algorithm"
        dummy_plugin_path.mkdir()
        (dummy_plugin_path / f"{dummy_module_name}.py").touch()
        (dummy_plugin_path / f"{dummy_module_name}.py").write_text(dummy_plugin)
        reload(bitfount.federated.algorithms)
    ...
    def test_algorithm_plugin_that_doesnt_subclass_base_alg_gets_ignored(
        ...
        reload(bitfount.federated.algorithms)
        # Tests that the module is still imported but the class is not
        algorithms = import_module(BITFOUNT_FEDERATED_ALG_MODULE_PATH)
    ...
    def test_algorithm_plugin_plugin_gets_loaded_via_api(
        ...
        federated = import_module(BITFOUNT_FEDERATED_MODULE_PATH)
        assert hasattr(algorithms, dummy_module_name)
        assert hasattr(federated, dummy_alg_plugin_name)
    ...
    def test_algorithm_plugin_gets_loaded_via_yaml(
        ...
        modeller_config = ModellerConfig(
            ...
            task=TaskConfig(
                protocol=ProtocolConfig(name="ResultsOnly"),
                algorithm=AlgorithmConfig(
                    name=dummy_alg_plugin_name, pretrained_file=Path("test.py")
                ),
                model=ModelConfig(
