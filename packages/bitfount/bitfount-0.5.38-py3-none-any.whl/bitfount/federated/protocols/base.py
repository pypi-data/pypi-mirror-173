...
class _BaseProtocolFactory(ABC, _RolesMixIn, _BaseSerializableObjectMixIn):
    ...
    def run(
        ...
        if isinstance(self.algorithm, _BaseModelAlgorithmFactory):
            if (
                len(pod_identifiers) > 1
                and hasattr(self.algorithm.model.datastructure, "table")
                and isinstance(self.algorithm.model.datastructure.table, str)
            ):
                raise ValueError(
            ...
            datastructure_pod_identifiers = (
                self.algorithm.model.datastructure.get_pod_identifiers()
            )
            if datastructure_pod_identifiers:
                datastructure_pod_identifiers = _check_and_update_pod_ids(
                    datastructure_pod_identifiers, hub
                )
                self.algorithm.model.datastructure._update_datastructure_with_hub_identifiers(
                    datastructure_pod_identifiers
                )
        if not session:
            session = hub.session
        ...
        result = modeller.run(pod_identifiers, require_all_pods=require_all_pods)
