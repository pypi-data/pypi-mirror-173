...
class AddAnchorTimeSparkNode(SparkExecNode):
    def __init__(self, node: AddAnchorTimeNode):
        ...
        self.batch_schedule = node.batch_schedule
...
class AddRetrievalAnchorTimeSparkNode(SparkExecNode):
    def __init__(self, node: AddRetrievalAnchorTimeNode):
        self.input_node = translate.spark_convert(node.input_node)
        self.feature_store_format_version = node.feature_store_format_version
        self.batch_schedule = node.batch_schedule
        self.tile_interval = node.tile_interval
        self.timestamp_field = node.timestamp_field
...
class ConvertEpochToTimestampSparkNode(SparkExecNode):
    def __init__(self, node: ConvertEpochToTimestampNode):
        self.input_node = translate.spark_convert(node.input_node)
...
class AddEffectiveTimestampSparkNode(SparkExecNode):
    def __init__(self, node: AddEffectiveTimestampNode):
        self.input_node = translate.spark_convert(node.input_node)
        self.timestamp_field = node.timestamp_field
