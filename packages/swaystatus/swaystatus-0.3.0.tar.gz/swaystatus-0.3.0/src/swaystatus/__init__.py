...
There are two primary ways of interacting with this package:
    ...
    1. Defining modules using the base class `swaystatus.element.BaseElement`
       to produce status bar blocks. For details, see the documentation for
       `swaystatus.element`.

    2. Producing encoded content for swaybar with the `swaystatus` command. For
       details on the command line interface, run `swaystatus --help`. For
       details on configuring swaystatus, see the documentation for
       `swaystatus.config`.

It should be made clear that this package does not contain any modules. It
simply provides a way to make them usable. One of the goals of this project was
to be as unopinionated as possible. I prefer simple, easy to read, modules, but
you may want colorful, feature-filled, eye candy. This package supports both,
