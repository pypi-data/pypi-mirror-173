...
def build_sfgwas() -> None:
    ...
    command = """export PYTHONUNBUFFERED=TRUE && export PATH=$PATH:/usr/local/go/bin && export GOPATH=/usr/local/go && export HOME=~ && export GOCACHE=~/.cache/go-build && cd sfgwas && go get -t github.com/simonjmendelsohn/sfgwas && go build"""
    run_command(command)
...
def start_sfgwas(role: str, demo: bool = False, protocol: str = "SFGWAS") -> None:
    ...
    command = f"export PYTHONUNBUFFERED=TRUE && export PATH=$PATH:/usr/local/go/bin && export GOPATH=/usr/local/go && export HOME=~ && export GOCACHE=~/.cache/go-build && cd sfgwas && {protocol_command}"
    run_command(command)
