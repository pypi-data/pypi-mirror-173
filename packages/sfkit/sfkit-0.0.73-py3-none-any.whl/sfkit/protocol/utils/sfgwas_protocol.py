...
def install_sfgwas() -> None:
    ...
    commands = [
        ...
        "unzip plink2_linux_avx2_20220603.zip",
        "mv plink2_linux_avx2_20220603 plink2",
        "pip3 install numpy",
    ]
...
def build_sfgwas() -> None:
    ...
    command = """export PYTHONUNBUFFERED=TRUE && export PATH=$PATH:/usr/local/go/bin && cd sfgwas && go get -t github.com/simonjmendelsohn/sfgwas && go build"""
    run_command(command)
...
def start_sfgwas(role: str, demo: bool = False, protocol: str = "SFGWAS") -> None:
    ...
    command = f"export PYTHONUNBUFFERED=TRUE && cd sfgwas && {protocol_command}"
    run_command(command)
