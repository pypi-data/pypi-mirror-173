...
struct DyadicHashReducer {
    ...
    uint64_t reduce(uint64_t full_hash) const {
        ...
        return fibmix(full_hash) & and_mask;
...
template<class K>
_HI_ uint64_t is_zero_key(const K& key, std::false_type) {
    return key.iszero();
}

template<class K>
_HI_ uint64_t is_zero_key(const K& key, std::true_type) {
    return (key == 0);
}
...
class indextable {
    ...
    K obtain_key(const T& element) const {
