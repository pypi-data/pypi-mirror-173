...
class SpecialInput(GeneralEnum):
    UNSPECIFIED = 0
...
class ConfigComponent(object):
    ...
    def has_default(self):
        # also check if None is one of the acceptable dtype
        return (self.default is not None) or (any(dtype.validate(None) for dtype in self.dtypes))

    def validate(self, value=None):
        if (value == SpecialInput.UNSPECIFIED):
    ...
    def parse_config(self, config:Union[Dict, str]):
        if isinstance(config, str):
            ...
            if not self.disable_message:
                self.stdout.info(f'INFO: Reading config file "{config}".')
            if extension == ".json":
                import json
            ...
            if isinstance(component, ConfigComponent):
                if key in config:
                    ...
                    if component.required_strict:
                        config[key] = component.validate(SpecialInput.UNSPECIFIED)
                        if not self.disable_message:
