...
class NumerousSolver:
    ...
    * :attr:`meth`: the solver method. Possible choices are 'RK45' :meth:`~solver.solver_methods.RK45`, \
    ...
    def __init__(self, model: Union[Model, object],
        ...
        if self.f0 is None:
            logger.info("Using no states solver. If this is not intended, make sure you specify the 'get_deriv' method "
                        "for the model")
            method = "NoDiff"
        try:
        ...
        class SolverEventHandler:
            """
            Class to allow model to return None for get_event_results
            """
            def __init__(self, interface: Interface, has_events: bool):
                self.interface = interface
                self._has_events = has_events
            def get_event_results(self, t, y):
                if self._has_events:
                    return self.interface.get_event_results(t, y)
                else:
                    return np.array([0], dtype='float')

        self.solvereventhandler = _JitHelper.jithelper(SolverEventHandler, jit=use_jit)(interface=self.interface,
                                                                             has_events=self.number_of_events > 0)
        ...
        self.interface.run_event_action(0, y0, -1)


    def _generate_solver(self):
        def _solve(interface, solvereventhandler, _solve_state, initial_step, dt_0, order, order_, roller, strict_eval,
            ...
            while solve_status != SolveStatus.Finished:
                ...
                if step_converged:
                    ...
                    if number_of_events > 0:
                        ...
                        g_new = solvereventhandler.get_event_results(t, y)
                        ...
                        for ix in np.concatenate((np.argwhere(up), np.argwhere(down))):
                            ...
                            status, t_event, y_event = check_event(solvereventhandler.get_event_results, ix[0],
                ...
                if number_of_events > 0 and min(t_events) < t:
                    ...
                    g = solvereventhandler.get_event_results(t_event, y_event)
                ...
                if not event_trigger and step_converged:
                    ...
                    if abs(t - t_next_time_event) < 1e-6:
                        ...
                        y_previous = interface.run_time_event_action(t, y, time_event_ix)  # todo: use interface to get y.
                ...
                if event_trigger:
                    ...
                    if abs(t_event - t_event_previous) > event_tolerance:
                        ...
                        dt = initial_step
                        g = solvereventhandler.get_event_results(t, y_previous)
                    else:
    ...
    def _solver(self, t_eval, info=None, delta_t=None):
        ...
        info = self._solve(self.interface, self.solvereventhandler, solve_state, initial_step, dt, order, order_,
        ...
        while info.status == SolveStatus.Running:
            ...
            info = self._solve(self.interface, self.solvereventhandler, info.solve_state, initial_step, info.dt, order,
