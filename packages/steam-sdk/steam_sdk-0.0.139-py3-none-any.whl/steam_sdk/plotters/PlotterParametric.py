...
class PlotterParametric:
    ...
    def plt_T_vs_I(self, hex_color_list):
        ...
        for i, magnet_label in enumerate(self.magnet_dict.keys()):
            ...
            ax1.scatter(I_mag, T_peak, color=hex_color_list[i], label=magnet_label)
            ...
            ax1.plot(xModel, yModel, color=hex_color_list[i], linestyle='dashed')
        ...
        ax1.set_ylabel(f"T max (K)", size=self.cs['font_size'])  # .set_fontproperties(font)
        ...
        plt.xticks(np.arange(150, 500, 50))
        #plt.yticks(np.arange(0, max(y), 2))
        #ax1.set_xlim(np.min(self.I_mag_dict[magnet_label]), np.max(self.I_mag_dict[magnet_label]))
        ax1.set_xlim(180, 460)
    ...
    def plot_b_and_w(self, hex_color_list):
        ...
        df = pd.DataFrame.from_dict({self.value: data, 'magnet': magnet_labels})
        ...
        for _, hex_color in zip(magnet_labels, hex_color_list):
            # plt.axhline(y=self.labels_dict[self.label]['limit'], color='#ff3300', linestyle='--', linewidth=1,
            #             label=f"Limit Value = {self.labels_dict[self.label]['limit']} {self.labels_dict[self.label]['unit']}")
            #ax = sns.boxplot(x='magnet', y=self.value, data=df, flierprops=flierprops, linewidth=0.5, showfliers=False, whis=[2.5, 97.5])#, palette="Set3")
            ax = sns.boxplot(x='magnet', y=self.value, data=df, flierprops=flierprops, linewidth=0.5, showfliers=False, whis=[2.5, 97.5])  # , palette="Set3")
        # for patch in ax.artists:
        #     rgb = rgb_list_b_and_w[0]
        #     #r, g, b, a = patch.get_facecolor()
        #     patch.set_facecolor((rgb['r'], rgb['g'], rgb['b'], 1))
        # ax = sns.violinplot(x="magnet", y=self.label, data=self.df, inner=None)
        # ax = sns.swarmplot(data=self.df, color=".25", s=1)
        plt.xlabel(magnet_type, fontweight='normal')  # fontsize=6)
        #plt.ylabel(self.value, fontweight='normal')  # fontsize=6)
        #plt.ylim(-510, 510)
        #plt.ylim(-45, 45)
...
if __name__ == "__main__":
    ...
    hex_color_list = ['#80A28D', '#8498B5', '#BF9382', '#9F80BE']
