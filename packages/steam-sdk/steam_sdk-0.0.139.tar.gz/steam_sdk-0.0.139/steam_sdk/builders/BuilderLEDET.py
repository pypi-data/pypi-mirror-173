...
class BuilderLEDET:
    ...
    def __init__(self, path_magnet: Path = None, input_model_data=None,
        ...
        if self.case_model == 'magnet':
            ...
            if flag_build:
                ...
                if type_windings != 'CCT_straight':  # avoid this step for CCT_straight as this reorders the thermal connections and creates issue for fqpls connection
                ...
                if self.model_data.GeneralParameters.magnet_type == 'CCT_straight':
                    self.setSelfMutualInductances()  # needs to be colleded before assignCCTValuesWindings as assignCCTValuesWindings needs to overwrite HalfTurnToInductanceBlock
    ...
    def assignCCTTurnsRefinement(self):
        ...
        if np.shape(self.Inputs.el_order_half_turns)[0] != np.shape(el_ord_csv)[0]:   # check if they are the same length (method below will not be able to check if they are the same if they are diffrent length)
            raise ValueError(f'There is mismatch between length of order calculated by BuilderLEDET: {np.shape(self.Inputs.el_order_half_turns)[0]} and in csv file: {np.shape(el_ord_csv)[0]}')
