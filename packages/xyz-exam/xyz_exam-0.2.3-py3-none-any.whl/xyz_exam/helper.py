...
def gen_paper_from_other(title, cond, other, order_number=0):
    ...
    gc = 0
    qc = 0
    for qt, qs in qts.items():
        gc += 1
        qc += len(qs)
        g = dict(title=group_name_normalize(qt), number=gc, type=qt, questionCount=len(qs), questions=qs)
        gs.append(g)
    cdata = dict(title=title, groups=gs)
...
def gen_chapter_papers(owner):
    ...
    for n in owner:
        ...
        for cn, cts in categories.items():
