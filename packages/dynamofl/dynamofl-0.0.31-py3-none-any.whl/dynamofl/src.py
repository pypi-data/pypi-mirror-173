...
class _Project(_Base):
    ...
    def push_model(self, path, datasource_key, params=None):
        ...
        with open(path, 'rb') as f:
            ...
            upload_url = self._make_request('GET', f'/projects/{self.key}/models/presigned-url', params=params)['uploadUrl']
            requests.put(upload_url, data=f.read())

            self._make_request('POST', url)

    def pull_model(self, filepath, datasource_key=None, round=None, federated_model=None, throw_error=False):
        params = {}
        if round is not None:
            params['round'] = round
