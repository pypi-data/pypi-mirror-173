...
TMP_DATA_SUB_FILE = 'iliadconn_data_sub.json'
...
def reload_old_data(seconds_delta_raise, exception_raise: Type[Exception] = ScrapingException):
    now = datetime.now()
    if os.path.exists(os.path.join(tempfile.gettempdir(), TMP_DATA_FILE)):
...
def calc_forecast(date_next_subscription, consumed, total_max, unsure):
    ...
    if date_now == date_end_current_subscription.date():
        with open(os.path.join(tempfile.gettempdir(), TMP_DATA_SUB_FILE), 'w+') as f:
            old_consumed_month = consumed
            if f_content := f.read():
                old_data_month = json.loads(f_content)
                old_consumed_month = int((old_data_month['consumed'] + consumed) / 2)
            f.seek(0)
            f.write(json.dumps({'consumed': old_consumed_month,
                                'date': dt_now.isoformat()}))
    if os.path.exists(os.path.join(tempfile.gettempdir(), TMP_DATA_SUB_FILE)):
        with open(os.path.join(tempfile.gettempdir(), TMP_DATA_SUB_FILE), 'r') as f:
            old_data = json.loads(f.read())
            old_date = datetime.fromisoformat(old_data['date'])
