...
def Success_Dag(ti,**context):
    ...
    dicc = {"dag":dag,"date_execution":exec_date,"duration":duration,"status":state,"date_log":end_date,"details":dim_ingestion}
    ...
    sql = f"""
        ...
        values ({dag},{exec_date},{duration},{state},{end_date},JSON_PARSE({dim_ingestion}))
        """.replace('insert',"insert").replace('update',"update").replace('delete',"delete").replace('ingestion',"ingestion")
