...
from rime_sdk.protos.ri.api.firewall.firewall_pb2 import Firewall as FirewallProto
from rime_sdk.protos.ri.api.firewall.firewall_pb2 import (
    FirewallComponents,
    FirewallRules,
    ...
    ListTestRunSummariesRequest,
    ListTestRunSummariesResponse,
    ReferenceSetSource,
    TimeRange,
...
from rime_sdk.protos.ri.api.jobs.jobs_pb2 import JobMetadata
...
class Firewall:
    ...
    def list_test_runs(self) -> Iterator[ContinuousTestRun]:
        ...
        req = ListTestRunSummariesRequest(firewall_id=self._firewall_id)
        with self._backend.GRPCErrorHandler():
            with self._backend.get_firewall_stub() as firewall_api:
                while True:
                    res: ListTestRunSummariesResponse = firewall_api.ListTestRunSummaries(  # pylint: disable=line-too-long
                        req
                    )
                    ...
                    next_start_time_sec = (
                        next_end_time.ToSeconds()
                        - LIST_TEST_RUNS_INTERVAL_LENGTH_SECONDS
                    )
                    next_start_time = Timestamp()
                    next_start_time.FromSeconds(next_start_time_sec)
                    next_interval = TimeRange(
                        start_time_epoch_seconds=next_start_time,
                        end_time_epoch_seconds=next_end_time,
                    )
                    req.interval.CopyFrom(next_interval)
