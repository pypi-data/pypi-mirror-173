...
class TestRun:
    ...
    def get_summary_tests_df(
        ...
        with self._backend.get_test_run_results_stub() as results_reader:
            ...
            page_token = ""
            while True:
                req = ListSummaryTestsRequest(page_size=20)
                if page_token == "":
                    req.query.test_run_id = self._test_run_id
                else:
                    req.page_token = page_token
                with self._backend.GRPCErrorHandler():
                    res: ListSummaryTestsResponse = results_reader.ListSummaryTests(req)
