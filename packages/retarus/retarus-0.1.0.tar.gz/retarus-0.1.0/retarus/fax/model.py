...
class SendRecipient(BaseModel):
    ...
    def __dict__(self):
        return {
            "number": self.number,
            "alternativeNumbers": self.alternative_numbers,
...
class Recipient(BaseModel):
    number: str = Field()
    alternative_numbers: Optional[str] = Field()
...
class Client(object):
    ...
    def get_fax_report(self, job_id: str) -> dict:
        """
        Get the fax report created by the servers with the job_id returned by the [send_fax] function.
        :param job_id:
