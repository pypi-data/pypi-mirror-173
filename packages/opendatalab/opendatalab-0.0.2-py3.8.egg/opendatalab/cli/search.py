...
def implement_search(obj: ContextInfo, keywords: str) -> None:
    ...
    if result_list:
        for _, res in enumerate(result_list):
            ...
            ds_data_types = ','.join([dmt['name'] for dmt in res['attrs']['mediaTypes']])
            ds_file_byte = bytes2human(res['attrs']['fileBytes'])
            ds_file_count = res['attrs']['fileCount']
            ds_task_types = ','.join([dtt['name'] for dtt in res['attrs']['taskTypes']])
            ds_task_types_rich = rich_content_str(keywords=keywords, content=ds_task_types)
            ds_label_types = ','.join([dlt['name'] for dlt in res['attrs']['labelTypes']])
