...
class _AWSStack(Stack):  # pylint: disable=too-many-instance-attributes
	def __init__(self, scope, _id, tool, config, data_urls, **kwargs):
		...
		self.config = config
		self.data_buckets = [
			data_url for data_url in data_urls if data_url.startswith('s3')
		]
		self.data_urls = [
			data_url for data_url in data_urls if data_url.startswith('http')
		]
		self.leader_name = ''
		self.nodes = []
		self.use_https = self.config['aws'].get('https', False)
	...
	def get_udata(self, _type):
		...
		if _type == 'leader':
			...
			self.add_leader_commands(
				...
				'cd /mnt/efs/data',
				*[f'aws s3 sync {data_bucket} /mnt/efs/data' for data_bucket in self.data_buckets],  # noqa: E501
