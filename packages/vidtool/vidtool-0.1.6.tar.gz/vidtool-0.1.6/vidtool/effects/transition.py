...
from ..tools import change_width_height_mp4, get_video_resolution
...
def get_path_file_output(path_file_max_transition, resolution):
    ...
    path_file_output = os.path.join(
        folder_script_path, path_dir, file_name_output
    )
...
def get_dict_path_file_transition_max_resiution():
    def get_path_file_transition_max_resiution(ratio):
        ...
        path_file_transition_max_resiution = dict_transition_resolution_filed[
            max_resolution
        ]
...
def get_dict_transition_resolution():
    ...
    if len(list_transition_path_file) == 0:
        raise Exception(
            "There is no transition video archived "
...
def check_transition_resolution(list_video_path_file):
    ...
    for resolution in resolutions_missing:
        ...
        path_file_output = get_path_file_output(
            path_file_transition_max_resolution, resolution
        )
