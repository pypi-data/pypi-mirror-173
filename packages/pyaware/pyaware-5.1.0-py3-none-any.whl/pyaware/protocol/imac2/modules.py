...
class ImacModule:
    ...
    def __init__(self, controller: Imac2MasterController, dev_id=""):
        ...
        self.parameter_handlers = {
            ...
            "external": self.parameter_poll_reader,
        }
        log.info(
    ...
    def setup_watchdogs(self):
        # Do not setup block read watchdogs for modules that do not have block reads (eg RTS)
    ...
    async def parameter_poll_reader(self, data: set) -> dict:
        ...
        if the last poll reading is current
        :param data: Set of parameters to read
        :return:
        """
        if self.last_poll_read + relativedelta(seconds=30) > datetime.utcnow():
            # Valid poll reading in the last 30 seconds
            common = data.intersection(
                self.parameters["poll"].keys()
                | self.parameters.get("external", {}).keys()
            )
            return {k: v for k, v in self.current_state.items() if k in common}
        else:
            return {}
