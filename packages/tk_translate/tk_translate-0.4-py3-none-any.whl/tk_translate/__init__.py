...
import sys, os, shutil, re, json, subprocess, warnings
...
layout = [[
    sg.Column([
        ...
        [
            ...
            #sg.Input(key="filename", visible=False),, enable_events=True, target="filename",
        ],
        # tabs
        [
   ...
   # add to *win_map{} event loop
    ...
    def file(self, data):
        fn = sg.tk.filedialog.askopenfilename(
            filetypes=[("Any", "*"), ("Office", "*.odt *.fodt"), ("Text", "*.txt *.md")]
        )
        #fn = sg.popup_get_file("Translate Office/text file to file.{lang}.ext", file_types=
        if not fn:
            return
    ...
    def file_text(self, fn, target_fn):
        with open(fn, "r") as read, open(target_fn, "w") as write:
            write.write(self.t.translate(read.read()))

    # flat content.xml
    def file_xml(self, fn, target_fn):
        with open(fn, "r") as read, open(target_fn, "w") as write:
            xml = read.read()
            xml = translationbackends.rx.xmltrans.sub(
                lambda m: m.group(1) + self.t.linebreakwise(m.group(2)),
                xml.decode("utf-8")
            )
            write.write(xml)
    ...
    def file_zip(self, fn, target_fn):
        ...
        with zipfile.ZipFile(target_fn, 'a') as zip:
            ...
            zip.writestr("content.xml", xml)  # retains the original content.xml, but LO loads the appended update
