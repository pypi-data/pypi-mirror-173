...
def _h5_data_to_np_array(h5f_input_data) -> Union[np.array, List[np.ndarray]]:  # type: ignore
    """
    Converts a single h5 data point to an array or list of arrays.
    """
    # Get the np array for each batch stored in the h5 file.
    curr_batches = []
    for i in range(0, int(h5f_input_data.attrs["batch_count"])):
    ...
    if h5f_data["0"].attrs.get("order"):
        ...
        for i in range(0, len(h5f_data.items())):
            ...
            for _, curr_data in h5f_data.items():
                if int(curr_data.attrs["order"]) == i:
