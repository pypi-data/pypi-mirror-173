...
class Plinux:
    ...
    def run_cmd(self, cmd: str, sudo: bool = False, timeout: int = 30, ignore_errors: bool = False) -> SSHResponse:
        ...
        try:
            ...
            msg_err = f'{self.host:<14} | {parsed.exited}: Something went wrong!\n\tSTDERR: {parsed.stderr}'
            if parsed.exited and not ignore_errors:  # Exit code != 0
            ...
            if parsed.stderr:  # Exit code != 0 and stderr contains message
                self.logger.warning(msg_err)  # Exit code != 0 and no stderr
    ...
    def compare_files(self, file1: str | Path, file2: str | Path, params: str = '', sudo: bool = False) -> bool:
        ...
        if not result.exited:
            self.logger.info('<- Files are equal')
            return True
        elif result.exited == 1:
