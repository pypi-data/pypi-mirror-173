...
class CfdiPago():
    ...
    def generar_cfdi(self, configuracion, timbrado_prueba=None, pac=None):
        ...
        if str(iva_p) == "0.160000":
            cp.TotalTrasladosBaseIVA16 = "%.2f" % (base_iva * tc)
            cp.TotalTrasladosImpuestoIVA16 = "%.2f" % (impuesto_iva * tc)

        if str(iva_p) == "0.080000":
            cp.TotalTrasladosBaseIVA8 = "%.2f" % (base_iva * tc)
            cp.TotalTrasladosImpuestoIVA8 = "%.2f" % (impuesto_iva * tc)
        
        if str(iva_p) == "0.00":
            cp.TotalTrasladosBaseIVA0 = "%.2f" % (base_iva * tc)
            cp.TotalTrasladosImpuestoIVA0 = "%.2f" % (impuesto_iva * tc)
