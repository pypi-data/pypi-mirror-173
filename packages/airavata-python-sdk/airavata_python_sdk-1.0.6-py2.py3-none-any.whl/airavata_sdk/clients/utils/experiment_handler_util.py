...
class ExperimentHandlerUtil(object):
    ...
    def launch_experiment(self, user_id, experiment_name="default_exp", description="this is default exp",
                          local_input_path="/tmp", file_mapping={}, computation_resource_name=None,
                          queue_name=None):

        access_token = self.access_token

        airavata_token = self.authenticator.get_airavata_authz_token(user_id, access_token,
        ...
        resource_host_id = airavata_util.get_resource_host_id(computation_resource_name)
        ...
        experiment = data_model_client.configure_computation_resource_scheduling(experiment_model=experiment,
                                                                                 ...
                                                                                 node_count=int(
                                                                                     self.experiment_conf.NODE_COUNT),
                                                                                 total_cpu_count=int(
                                                                                     self.experiment_conf.TOTAL_CPU_COUNT),
                                                                                 wall_time_limit=int(
                                                                                     self.experiment_conf.WALL_TIME_LIMIT),
        ...
        if (len(file_mapping.keys()) > 0):
            ...
            for key in file_mapping.keys():
                if type(file_mapping[key]) == list:
                    data_uris = []
                    for x in file_mapping[key]:
            ...
            experiment = data_model_client.configure_input_and_outputs(experiment, input_files=None,
                                                                       application_name=self.experiment_conf.APPLICATION_NAME,
        ...
        if self.experiment_conf.MONITOR_STATUS:
            ...
            sftp_connector.download_files(".", path)
