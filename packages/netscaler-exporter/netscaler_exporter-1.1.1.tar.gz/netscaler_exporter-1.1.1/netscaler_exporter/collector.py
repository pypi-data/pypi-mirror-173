...
#******************************************************************************************
class NetscalerCollectorRequestHandler(BaseHTTPRequestHandler):
...
#        self.server.collector.exporter.logger.debug('GET : {0}'.format( message ) )
        if url.path == '/metrics':
            ...
            if 'target' in params:
               ...
               if not found:
                ...
                self.end_headers()
                output = bytes("""<html>
   <head><title>Netscaler Exporter</title></head>
   <body>
...
def handler(*args, **kwargs):
    NetscalerCollectorRequestHandler(*args, **kwargs)
