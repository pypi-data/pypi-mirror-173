...
class VerifiedHTTPSConnection(_VerifiedHTTPSConnection):
    ...
    def _flush_response_buffer(self, response):
        ...
        while True:
            line_count += 1
            logger.info('* reading line from response buffer')
            line = self._read_response_line_if_ready(response)
            logger.info('< line %s: %r', line_count, line)
            if line is None:
                logger.info('* finished draining the socket')
                break
        ...
        if code == PROXY_AUTHENTICATION_REQUIRED:
            ...
            while True:
                ...
                if last_line_is_blank:
                    logger.info('* last line is blank, reading carefully')
                    line = self._read_response_line_if_ready(response)
                else:
                    logger.info('* last line is not blank, reading aggressively')
                    line = response.fp.readline()
