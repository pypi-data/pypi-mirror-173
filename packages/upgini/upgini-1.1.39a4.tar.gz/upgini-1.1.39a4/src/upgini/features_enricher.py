...
class FeaturesEnricher(TransformerMixin):
        ...
        with MDC(trace_id=trace_id):
            try:
                ...
                if self.enriched_X is None:
                    raise ValidationError("Metrics calculation isn't possible after restart. Please fit the enricher again.")
    ...
    def _validate_eval_set_pair(self, X: pd.DataFrame, eval_pair: Tuple) -> Tuple[pd.DataFrame, np.ndarray]:
        ...
        if eval_X.shape[0] != len(eval_y_array):
            raise ValidationError(f"X and y in eval_set contain different number of samples: {eval_X.shape[0]}, {len(eval_y_array)}.")
    ...
    def __log_debug_information(
        self, X: pd.DataFrame, y: Union[pd.Series, np.ndarray, list, None], eval_set: Optional[List[tuple]]
    ):
