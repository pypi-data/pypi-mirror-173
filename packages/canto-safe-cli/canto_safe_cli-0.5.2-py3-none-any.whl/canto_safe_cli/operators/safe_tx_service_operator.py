...
from canto_safe_cli.api.base_api import BaseAPIException
from canto_safe_cli.utils import yes_or_no_question
...
class SafeTxServiceOperator(SafeOperator):
    ...
    def add_delegate(self, delegate_address: str, label: str, signer_address: str):
        ...
        elif signer_address not in self.canto_safe_cli_info.owners:
    ...
    def execute_tx(self, safe_tx_hash: Sequence[bytes]) -> bool:
        ...
        elif len(safe_tx.signers) < self.canto_safe_cli_info.threshold:
            print_formatted_text(
                HTML(
                    f"<ansired>Number of signatures {len(safe_tx.signers)} "
                    f"must reach the threshold {self.canto_safe_cli_info.threshold}</ansired>"
                )
            )
        else:
