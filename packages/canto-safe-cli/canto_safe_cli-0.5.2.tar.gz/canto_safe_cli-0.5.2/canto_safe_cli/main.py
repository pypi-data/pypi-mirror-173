...
from canto_safe_cli.operators import (
    SafeOperator,
    SafeRelayOperator,
    SafeServiceNotAvailable,
    SafeTxServiceOperator,
)
from canto_safe_cli.prompt_parser import PromptParser, to_checksummed_ethereum_address
from canto_safe_cli.safe_completer import SafeCompleter
from canto_safe_cli.safe_lexer import SafeLexer
...
class SafeCli:
    ...
    def get_bottom_toolbar(self):
        return HTML(
            ...
            f"{self.safe_operator.canto_safe_cli_info}</style></b>"
        )
        ...
        while True:
            try:
                ...
                if new_operator:
                    ...
                    new_operator.refresh_canto_safe_cli_info()  # ClI info needs to be initialized
                else:
...
def main(*args, **kwgars):
    ...
    """
    canto_safe_cli = SafeCli()
    canto_safe_cli.print_startup_info()
    canto_safe_cli.loop()
