...
The easiest solution is to use a different port for communication between the client and scheduler. In the following example, you can use port 443, which is usually not blocked since it is used for HTTPS. You'll need to tell Dask to use 443 instead of 8786 as the scheduler port and you'll need to tell Coiled to open port 443 on the cluster firewall  we create. You can do this with the ``scheduler_options`` and ``backend_options`` keyword arguments, respectively.
...
.. code-block:: python
    ...
    cluster = coiled.Cluster(
        n_workers=1,
        scheduler_options={"port": 443},
        backend_options={"ingress": [{"ports": [8787, 443], "cidr": "0.0.0.0/0"}]},
    )
