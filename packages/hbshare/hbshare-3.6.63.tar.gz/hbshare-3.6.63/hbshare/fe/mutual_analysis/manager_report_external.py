...
class QuantitativeReport:
    def __init__(self, file_path, manager_code, fund_code=None):
        ...
        if len(self.change_dates) == 1:
            self.change_dates = []
             ...
             self.change_dates = [datetime.strptime(date, '%Y%m%d') + timedelta(45) for date in self.change_dates][:-1]
    ...
    def get_representative_fund(self):
        ...
        if self.fund_code is not None:
            ...
            while not i == len(representative_fund):
                ...
                representative_fund.loc[i: fund_idx + 1, 'FUND_CODE'] = fund_code
                i = fund_idx + 1
    ...
    def get_manager_scale(self):
        ...
        change_dates = [date for date in self.change_dates if date >= min(manager_fund_scale['REPORT_DATE']) and date <= max(manager_fund_scale['REPORT_DATE'])]
        change_dates_loc = get_change_dates_loc(change_dates, sorted(manager_fund_scale['REPORT_DATE'].unique().tolist()))
        ...
        ax1.set_ylabel('')
        ax2.set_ylabel('')
        ax1.vlines(x=change_dates_loc, ymin=0.0, ymax=max(manager_fund_scale['FUND_SCALE']), linestyles='dashed', color='#959595')
    ...
    def get_manager_size(self):
        ...
        for col in list(hbs_size_history.columns[3:]):
    ...
    def get_manager_style(self):
        ...
        for col in list(hbs_style_history.columns[3:]):
    ...
    def get_manager_theme(self):
        ...
        for idx, row in representative_fund.iterrows():
            ...
            hbs_theme_history = FEDB().read_industry_exposure_given_code(fund_code)
            hbs_theme_history = hbs_theme_history.rename(columns={'jjdm': 'FUND_CODE', 'jsrq': 'REPORT_DATE', 'yjxymc': 'INDUSTRY', 'zjbl': 'INDUSTRY_EXPOSURE'})
            hbs_theme_history['THEME'] = hbs_theme_history['INDUSTRY'].apply(lambda x: industry_theme_dic[x])
            hbs_theme_history = hbs_theme_history[['FUND_CODE', 'REPORT_DATE', 'THEME', 'INDUSTRY_EXPOSURE']].groupby(['FUND_CODE', 'REPORT_DATE', 'THEME']).sum().reset_index().rename(columns={'INDUSTRY_EXPOSURE': 'THEME_EXPOSURE'})
        ...
        for col in list(hbs_theme_history.columns[3:]):
    ...
    def get_manager_industry(self):
        ...
        for idx, row in representative_fund.iterrows():
            ...
            hbs_industry_history = FEDB().read_industry_exposure_given_code(fund_code)
            hbs_industry_history = hbs_industry_history.rename(columns={'jjdm': 'FUND_CODE', 'jsrq': 'REPORT_DATE', 'yjxymc': 'INDUSTRY', 'zjbl': 'INDUSTRY_EXPOSURE'})
        ...
        for col in list(hbs_industry_history.columns[3:]):
    ...
    def get_manager_performance(self):
        ...
        sns.lineplot(x='TRADE_DATE', y=index_name, data=fund_index_cumret, color='#595959', label=index_name)
        ...
        nav_df = fund_nav.merge(index_daily_k, left_index=True, right_index=True, how='left').merge(stock_fund_index_daily_k, left_index=True, right_index=True, how='left')
    ...
    def get_manager_performance_all(self):
        ...
        for idx, row in manager_fund.iterrows():
            ...
            manager_fund_scale_code = HBDB().read_fund_scale_given_code(fund_code)
            manager_fund_scale_code = manager_fund_scale_code if len(manager_fund_scale_code) > 0 else pd.DataFrame(columns=['ZCJZ', 'BBLB1', 'JSRQ', 'JJDM', 'GGRQ', 'ROW_ID'])
            manager_fund_scale_code = manager_fund_scale_code[manager_fund_scale_code['BBLB1'] == 13]
            manager_fund_scale_code = manager_fund_scale_code.sort_values(['JJDM', 'JSRQ', 'GGRQ']).drop_duplicates(['JJDM', 'JSRQ'], keep='last')
            fund_scale_list.append(manager_fund_scale_code)
        ...
        fund_scale = pd.concat(fund_scale_list)
        fund_scale = fund_scale.rename(columns={'JJDM': 'FUND_CODE', 'JSRQ': 'REPORT_DATE', 'ZCJZ': 'AUM'})
        fund_scale['REPORT_DATE'] = fund_scale['REPORT_DATE'].astype(str)
        ...
        fund_nav.loc[manager_fund['BEGIN_DATE'].min()] = 0.0
        fund_nav = fund_nav.sort_index().reset_index()
        fund_nav['NAV'] = (fund_nav['RET'] + 1).cumprod()
        fund_nav['CUM_RET'] = fund_nav['NAV'] / fund_nav['NAV'].iloc[0] - 1.0
        fund_nav['IDX'] = range(len(fund_nav))
