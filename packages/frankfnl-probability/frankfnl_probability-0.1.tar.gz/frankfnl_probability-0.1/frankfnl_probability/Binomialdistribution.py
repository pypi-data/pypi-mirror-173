...
class Binomial(Distribution):
    ...
    def plot_bar_pdf(self):
        ...
        k_values = list(range(self.n + 1))
        dist_values = [binom.pmf(k, self.n, self.p) for k in k_values]
        plt.bar(k_values, dist_values)
        plt.xlabel('n')
        plt.ylabel('probability density function')
        plt.title('Binomial Distribution')
        plt.show()
        return k_values, dist_values
