...
from .engineapi.typedefinitions import (ActivityReference, ExecuteInspectorCommonArguments,
                                        ExecuteInspectorsMessageArguments, InspectorArguments,
                                        ExecuteInspectorCommonResponse, RowsData, ColumnDefinition, FieldType,
                                        DataField)
...
class TableInspector(BaseInspector):
    ...
    def __init__(self, includeSTypeCounts: bool, includeAverageSpacesCount: bool, includeStringLengths: bool,
...
def get_column_type(batch, column):
    ...
    filtered_kinds = [value_kind_to_field_type[k[0]] for k in value_kinds if k[0] != 0 and k[0] != 9]
...
def get_data_quality_aggregate(batch, column, aggregate, total_count):
    ...
    if aggregate == 0:  # not missing
        ...
        try:
            value = next((k[1] for k in batch['valueKinds'][0][column] if k[0] == 9))
...
def get_histogram(batch, column):
    ...
    return DataField(FieldType.LIST, [to_data_field(v) for v in histogram_values])


# This list is mirrored in TableInspectorDefinition.cs and must be kept in sync.
profile_columns = {
...
def get_inspector_response_from_batches(batches):
    if len(batches) == 0:
        return ExecuteInspectorCommonResponse(column_definitions=[], rows_data=RowsData([]))
    elif len(batches) > 1:
    ...
    def get_column_aggregates_row(column):
        return [get_fn(aggregates, column, total_count) for _, get_fn in profile_columns.items()]

    inspector_column_definitions = [ColumnDefinition(c, FieldType.STRING) for c in profile_columns.keys()]
    inspector_rows = [get_column_aggregates_row(c) for c in columns_aggregated]
    return ExecuteInspectorCommonResponse(column_definitions=inspector_column_definitions,
...
class _Inspector:
    ...
    def _from_execution(cls,
        ...
        try:
            script = engine_api.get_inspector_lariat(ExecuteInspectorCommonArguments(
                context=context,
                inspector_arguments=inspector_arguments,
                offset=0,
                row_count=_MAX_ROW_COUNT))
            executor = get_rslex_executor()
            (record_batches, _) = executor.execute_dataflow(script,
                                                            True,
                                                            fail_on_error=False,
